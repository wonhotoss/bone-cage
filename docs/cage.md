...
`lift = max(0, max(바닥·축) − min(올리는 것·축))`을 **올리는 것 전체에 같은 값으로** 더한다 — 뭉치가 통째로 움직이므로 그것이 이루는 모양은 그대로다.

| 이름 | 올리는 것 | 바닥 | 축 | 비고 |
|---|---|---|---|---|
| `head above arms` | crown 링, head 링, `crown mid`, `head mid` (정점 12) | L·R arm 링의 hi 변 (정점 4) | `up` | 목을 줄이면 파팅 평면의 최하단(앞 두 코너)이 이음선 아래로 내려가 목 판이 머리를 뚫는다 — 머리 뭉치를 어깨 위로 되올린다 `[N20]` |

rest에서는 머리가 이미 어깨 위에 있어 `lift = 0`이므로 **rest 케이지는 보정되지 않는다**(분기 없이 성립).
...
- **[N20] 맞추는 대신 지킨다 — 보정(gate).** 케이지의 모양은 **주어진 메시에 의존적**이다. 망치상어처럼 머리가 좌우로 긴 모델, 어깨에 돌출부가 있는 모델을 주면 "살에 꼭 맞춰 자기겹침을 없앤다"는 목표 자체가 도달 불가능해진다 — 어떤 메시는 언제나 여유를 빼앗아 간다. 그래서 목표를 바꾼다: **비상식적인 스켈레톤 변화에서 케이지를 지킨다.** 그런 변화가 실제로 요구되면 그때 케이지를 고도화해도 늦지 않다.
    ...
    - **rest는 보정하지 않는다.** rest에서 머리는 이미 어깨 위이므로 `lift = 0`이고 보정이 no-op이다 — 특별 분기 없이 성립한다. bake의 측정도 rest 살에서 하므로 영향이 없다.
