...
링과 기둥이 **전부 놓인 뒤** 도는 단계. 링이 기둥에 걸리는 것(`hold`, `[N16]`)은 기둥이 링보다 먼저 놓인다는 한 방향 의존으로 풀렸지만, 링 대 링은 그렇게 풀 수 없다 — 그래서 보정은 케이지 전체를 읽고 고친다 `[N20]`. **표의 순서가 우선순위다**: 뒤의 보정이 앞의 보정이 옮긴 정점을 다시 옮길 수 있다.
...
rest에서는 머리가 이미 어깨 위에 있고 arm 링 hi 변도 머리 폭 바깥(Arm 관절 side 0.16 대 머리 반폭 약 0.09)이라 세 보정 모두 `lift = 0`이므로 **rest 케이지는 보정되지 않는다**(분기 없이 성립).
...
- **자기겹침 지도.** 스윕(§7b)이 판정하는 유일한 기준이므로 남은 자리를 여기 적어 둔다. 측정 2026-09-05, 손을 rest에 묶고 전신 무작위 20,000 + 쌍 1,012 + 단독 184, seed 1, **첫 보정(gate) 여유 0**(`512d81a`; 여유 38 mm의 전신은 아직 재지 않았다 — 1·2층은 케이스 수가 같고 쇄골 무리의 삼각형만 5 → 7로 올랐다). **단독은 실패 0**, 쌍은 7건(쇄골 4 · 골반 3), 전신은 **2,837 / 20,000**(보정 전 5,277). 보정이 목이 가라앉아 생기던 것을 함께 닫아 A는 −60%~전멸, B −84%, C −71%이고 **남은 것의 대부분이 D**다.

    | 기전 | 그룹(케이스) | 방아쇠 | 뿌리 |
    |---|---|---|---|
    | **D 골반·다리 폭** | `spine+spine mid+spine1` 1,670 · `L/R hip+L/R knee` 1,023·1,058 · `ankle+knee` 762 · `knee+crotch` 484 | 고관절 뼈 0.5(단독은 무사, pelvis나 반대쪽과 겹치면 깨진다) | 고관절 기둥만 뼈를 (1+f)배로 따라간다. `L hip=0.5`에서 그 기둥이 side 0.1809 → 0.0905로 걸어 들어오는데 spine 링의 변은 0.1635, 무릎 링은 rest 폭에 그대로 남는다 — 골반 판의 변이 꺾이고 허벅지 판의 옆변이 뒤집힌다. `[N16]`이 **높이**는 닫았지만 **폭**은 열려 있다. 두께 driver와 같은 뿌리이고, 다음 gate의 후보다 |
    | **A 이음선 대 턱선** | `head+head mid+neck mid` 822 (`crown+head`는 3,036 → **0**) | 목뼈 셋 중 하나 0.5, 또는 어깨밑동 1.5 | arm 링 hi 변이 목뼈를 따라가지 않는다. 보정이 머리를 어깨 위로 되올려 대부분 닫혔고 `[N20]`, 남은 것은 쇄골이 방아쇠인 것들이다 |
    | **C 어깨 쐐기** | `L/R arm+L/R delt` 381·421 | 쇄골 0.5 + 어깨밑동 1.5, 또는 chest 0.5 | 쇄골이 짧아지면 이음선이 정중선 쪽으로 오는데 `delt`는 상완에 매달려 밖에 남아 쐐기 삼각형이 뒤집힌다 `[N15]` |
    | **B 가슴 띠 뒤틀림** | `L/R arm+sternum mid` 207·206 (3그룹 짜리는 833·838 → **0**) | chest 0.5 + 쇄골 | `[N17]`의 두 여유가 두 가로대의 깊이를 13 cm 벌려 놓아 사각형이 안장이 된다. 그 사이에 가로대를 하나 넣으면 닫힌다 — 다만 보정 뒤 남은 양이 6분의 1이라 급하지 않다 |
    | **E 머리 판 안장** | **0** (`try-neck-fit`에서 1,451·1,445) | — | head 링을 목에 맞춰 좁히면 crown 캡과의 사이가 안장이 되던 것. 이 줄기는 링을 좁히지 않고 보정으로 지키므로 기전 자체가 없다 |

    **쇄골이 방아쇠인 넷**(A·B·C에 걸쳐 있다)은 하나의 뿌리를 공유한다 — `[N11]`의 `arm outward hi = −0.05`가 **절대 거리**라, 쇄골이 짧아지면 그 5 cm가 `neck mid`를 넘는다. 비율이어야 할 값이 절대값인 것이고 §9 두께 driver와 같은 종류다(씬은 지금 0으로 물렸다). §6b `L/R arm beside head`가 이것을 닫는다(2026-09-06) — 스윕 재확인은 아직이다. 남는 것이 있으면 lo 변(겨드랑이)의 `outward lo`도 같은 절대값이므로 B 무리는 따로 본다.
