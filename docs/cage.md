...
- **자기겹침 지도 — 1·2·4층은 닫혔고, 3층에 V 바닥 14(G′의 나머지)가 열려 있다.** 2026-09-07, 네 보정이 모두 든 케이지로 잰 스윕(§7b, 손을 rest에 묶고 seed 1):
...
- **V넥 다듬기.** (1) arm·hi의 높이는 아직 어깨 관절 평면에서 잰 삼각근 정점 높이 — 승모근 능선에 딱 맞추려면 변별 측정 창이 필요. (2) 가슴 띠와 V 바닥의 깊이는 이제 몸통 공유 깊이 하나다(§3f, `[N28]`) — 가슴이 새면 `body front`, 등이 새면 `body back`. arm 링·neck mid에 자기 깊이 튠이 없다. (3) `neck mid`의 높이는 **Neck 관절 그 자리로 확정**(2026-09-09) — V 라인이 목 판과 몸통 판을 가르는 자리이므로 목뼈를 따라야 하고, 승모근 선의 교점으로 내리지 않는다. 깊이는 몸통 공유(§3f). chest를 줄여 V 바닥이 이음선 아래로 내려가는 자기겹침은 **gate로 막는다**(§6b에 추가 예정: 올리는 것 `neck mid`, 재는 곳 그 두 끝, 바닥 L·R arm hi 변, 여유 = rest 차 약 2 cm). (4) head 링의 tilt·offset은 head splitter에서 읽은 초기값(25°, 0.023) — 튠 확정 후 표로 옮기고 씬의 splitter 오브젝트는 지운다. (5) 목을 줄일 때 이음선이 턱선을 넘는 것은 아직 열려 있다(§9 자기겹침 지도 A).
