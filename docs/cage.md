...
**케이스 검사** — `--case "<이름>"`(`results.csv`의 case 열 그대로, `--tiers`·`--skip`은 그 케이스가 든 층으로)은 한 케이스의 자기겹침 삼각형을 정점 번호·이름·위치(cm, spine 링 프레임의 side / up / depth)로 풀어 쓰고, 열린 지도 주변의 랜드마크(crown·head·L/R arm·정중선 기둥·왼발)를 함께 찍는다. 보고서는 그룹 이름까지만 말하므로 기전을 읽을 때 쓴다(2026-09-12, 지도 G·H). 무작위 케이스(`random#i`)는 이름이 없으므로 rest에서 ¼ 이상 벗어난 뼈를 함께 적는다.
