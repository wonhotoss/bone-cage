...
- **표가 선언하는 것은 고리와 사슬뿐이다**(목 반판 둘의 시작점이 예외로 대각선까지 정한다, `[N3]`)**.** 판을 채우는 ladder의 가로대(`v[i]`–`v[j]` 짝)와 쿼드를 가르는 대각선은 `strip`의 선택이며 어느 행도 이름하지 않는다. 그래서 사슬의 연속 기둥 쌍 — 그것이 곧 껍질의 링이고, 기울어진 고관절 링(`crotch` 옆 `L/R hip`)과 어깨 링(`L/R delt` 옆 arm·lo)이 여기서 나온다 — 을 `cage_constants.grid`에 정점 쌍으로 남긴다. 삼각화는 이 구분을 지우므로 `tris`에서 되돌릴 수 없다. 디버그 와이어(§7)가 읽는다.
...
- **자기겹침 지도.** 스윕(§7b)이 판정하는 유일한 기준이므로 남은 자리를 여기 적어 둔다. 측정 2026-09-04, 전신 무작위 20,000 + 쌍 1,012, seed 1, 그때의 튠. 1층 단독으로 깨지는 본은 없다. 네 기전이고, 셋은 **한 제어점만 뼈를 따라가고 이웃이 따라가지 않는다**는 같은 형태다 — `[N16]`·`[N18]`이 각각 그 한 사례를 닫았다.

    | 기전 | 그룹(케이스) | 방아쇠 | 뿌리 |
    |---|---|---|---|
    | **A 이음선 대 턱선** | `crown+head` 3,036 · `head+head mid+neck mid` 2,073 · crown 계열 422 | 목뼈 셋 중 하나 0.5, 또는 어깨밑동 1.5 | arm 링 hi 변이 목뼈를 따라가지 않는다. 답은 `[N18]`의 걸림(측정: 1·2층 자기겹침 50 → 3 케이스)이거나, 목에 자기 링을 주어 간접으로 따라가게 하는 것 |
    | **B 가슴 띠 뒤틀림** | `L/R arm+sternum mid` 1,283·1,324 · `L/R arm+neck mid+sternum mid` 833·838 | chest 0.5 + 목뼈나 어깨밑동 | `[N17]`의 두 여유가 두 가로대의 깊이를 13 cm 벌려 놓았다. `chest=0.5 + 아랫목=0.5`에서 윗 가로대가 실루엣→정중선으로 10.9 cm 내려가고 그 끝이 아래 가로대보다 4.9 cm 높은데 8 cm 뒤에 있다 — 사각형이 안장이 되어 접힌다. 그 사이에 가로대를 하나 넣으면 닫힌다 |
    | **C 어깨 쐐기** | `L/R arm+L/R delt` 1,354·1,380 | 쇄골 0.5 + 어깨밑동 1.5, 또는 chest 0.5 | 쇄골이 짧아지면 이음선이 정중선 쪽으로 오는데 `delt`는 상완에 매달려 밖에 남아 쐐기 삼각형이 뒤집힌다 `[N15]` |
    | **D 골반·다리 폭** | `spine+spine mid+spine1` 1,670 · `L/R hip+L/R knee` 1,023·1,058 · `ankle+knee` 762 · `knee+crotch` 484 | 고관절 뼈 0.5(단독은 무사, pelvis나 반대쪽과 겹치면 깨진다) | 고관절 기둥만 뼈를 (1+f)배로 따라간다. `L hip=0.5`에서 그 기둥이 side 0.1809 → 0.0905로 걸어 들어오는데 spine 링의 변은 0.1635, 무릎 링은 rest 폭에 그대로 남는다 — 골반 판의 변이 꺾이고 허벅지 판의 옆변이 뒤집힌다. 두께 driver와 같은 뿌리다 |
...
- **여유(clearance).** rest에서는 **36,426 정점 전부가 케이지 안**이고 자기겹침도 없다. 남은 문제는 얼마나 안에 있는가다 — `[N18]`의 한계값은 0.5 mm인데 지금 가장 가까운 정점이 **0.045 mm**다(측정 2026-09-04). 한계값 아래인 곳: 손가락 마디 전부(0.045~0.47 mm, 가장 가까운 것이 index1·middle1·pinky1), 머리 판(0.260 mm, `crown mid+head+head mid`), 골반 판(0.298 mm, `L hip+crotch+spine mid`). 나머지 부위는 0.9 mm 이상이다. 원인은 `margin`이 **비율**이라는 것이다 — 반폭 7 mm인 손가락에 5%는 0.35 mm뿐이고, 둥근 단면을 감싼 사각 링은 변의 중앙에서 살에 가장 가깝다. 포함 검사가 이제 이 여유를 재므로(§7) 편집 중에 바로 보인다. 남은 일은 레시피가 그것을 스스로 지키게 하는 것 — `inflate`에 절대 하한을 두어 작은 부위도 한계값 위로 올린다. 머리·골반은 룰이 아니라 튠이 판을 살에 붙인 것이라 그 값을 다시 본다.
