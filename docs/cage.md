...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `crown` | Head | Head | +up | side | depth | cap | 0 | 0 | | | | | 키 | 정수리 캡. 폭·높이가 키를 따른다 `[N23]`, 깊이는 복원(§6)이 그 폭에서 — 자리 Head `[N29]`. `front`·`back`은 튠 중(§7) — 가슴~배꼽 정중선과 날개뼈가 몸통 판을 뚫음 |
| `head` | Head | Head | up↷25° | side | depth↷25° | split | 0 | 0 | | | 0.023 | 0.023 | 키 | 머리–목 분리 평면. 폭과 오프셋이 키를 따른다 `[N23]`, 깊이는 복원(§6) — 자리 Head `[N29]`. 기울기·오프셋은 씬의 head splitter에서 읽음; 기울기·오프셋·`front`·`back` 튠 중(§7) `[N12]` |
...
| `L ankle` | LeftFoot | LeftLeg | −up↷45° | side | depth↷45° | joint | 튠(§7) | 튠(§7) | | | | | LeftUpLeg | 발목. Foot 관절을 지나 뒤로 기울어진 링 — 뒤꿈치에서 발등–정강이 연결부로. 기울기·`front`(발등 쪽)·`back`(뒤꿈치 쪽) 튠 중(§7) `[N14]`. 복원(§6)의 자리는 Foot — 뒤꿈치 코너가 발바닥과 같은 양만큼 내려간다 `[N29]` |
...
| `body front` / `body back` | 몸통 살의 `depth` 구간을 inflate한 앞끝/뒤끝 − Hips 좌표 + 튠 `body front`/`body back`(씬, 튠 중 §7, 초기 0 / 0). d 앵커는 항상 **Hips** |
...
|---|---|---|---|---|---|---|
| `L arm above spine2` / `R arm above spine2` | 그쪽 arm 링 전체 (정점 4) | 그 링의 lo 변 — 겨드랑이 (정점 2) | spine2 링의 그쪽 코너 둘 | 튠 중(§7, 초기 −0.01) | `up` | 어깨 밑동·upper thorax를 줄이고 쇄골을 늘이면 겨드랑이(이음선 절반 아래)가 Spine2까지 내려온다. 절두체의 교점이 끝에 붙어 spine2 링이 겨드랑이 코너와 한 자리가 되고, 복원이 둘의 깊이를 따로 놓으면 그 사이 띠가 접혀 몸통 반판·상완 판을 뚫는다(§9 지도 H). 재는 곳은 겨드랑이 변, 세우는 것은 **링 전체**라 이음선 기울기가 남는다. **여유는 음수여야 한다** — clamp된 상태에서 바닥은 겨드랑이의 옛 자리 그 자체라, 여유가 곧 벌어지는 띠의 높이다. `head above arms`보다 먼저 돈다(그것이 읽는 어깨를 옮기므로) `[N20]` |
| `neck mid above sternum` | `neck mid` 두 끝 (정점 2) | 그 둘 | `sternum mid` 두 끝 | 튠 중(§7, 초기 −0.02) | `up` | chest를 줄이면 Neck 관절과 함께 V 바닥이 겨드랑이 선으로 내려오고, 어깨 밑동이 길고 쇄골이 짧으면 겨드랑이와 그 교점(`sternum`)이 올라온다 — sternum이 V 바닥 위에 서면 몸통 꼭대기 가로대 둘이 순서를 바꿔 목 판이 가슴 띠를 뚫는다(§9 지도 G′ V 바닥). 기둥만 올린다(이음선 변은 그대로). **여유는 음수** — V의 높이를 요구한다. rest 차 12.6 cm `[N20]` |
| `head above arms` | crown 링, head 링, `crown mid`, `head mid` (정점 12) | head 링 네 코너 중 최하단 | L·R arm 링의 hi 변 (정점 4) | 튠 중(§7, **0.01** — 2026-09-12; 처음 0.038, 씬 0.03이었다) | `up` | 목을 줄이면 파팅 평면이 이음선 아래로 내려가 목 판이 머리를 뚫는다 — 머리 뭉치를 어깨 위로 되올린다 `[N20]` |
| `L arm beside head` / `R arm beside head` | 그쪽 arm 링 전체 (정점 4) | 그 링의 hi 변 (정점 2) | head 링 네 코너 중 그쪽으로 가장 바깥 | 튠 중(§7, **−0.03** — 2026-09-13; 처음 0, 씬 −0.01이었다) | `+side` / `−side` | 쇄골을 줄이면 arm 링이 관절을 따라 정중선 쪽으로 오다 hi 변(승모근 점)이 머리 실루엣 안에 선다 — 옆벽이 안쪽으로 눕고 목 반판이 머리 반판을 뚫는다. 재는 곳은 hi 변, 세우는 것은 **링 전체**라 라글란 기울기가 남는다. head gate와 축이 직교해 순서 무관 `[N20]` |
