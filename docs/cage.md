...
- **자기겹침 지도 — 전부 닫혔다.** 2026-09-07, 네 보정이 모두 든 케이지로 잰 스윕(§7b, 손을 rest에 묶고 seed 1):
    ...
    | **G V 삼각형이 머리 판을 뚫는다** (열림, 2026-09-12, **4층**) | 몸통 반판 꼭대기 `L/R arm+neck mid+sternum mid` 2·2 · 머리 판 `crown+head`·`crown(mid)+head(mid)` 4·2·2 (2~4 삼각형) | `torso 0.7 · arms 0.85 · legs 1.4` + 좌우 한쪽 0.9/1.1 (좌우 1·1은 통과) | 머리 깊이 복원이 열었다(직전 스윕은 통과). 키 비 1.124로 머리가 세 축 모두 커지는데 목·척추는 30% 짧다. head 링의 턱 쪽 여유는 25° 기울어진 d를 따라 앞·아래로 커지고, `head above arms`는 head 링 최하단을 arm hi 변 −3.8 cm까지만 지키므로 커진 머리 판이 V 삼각형과 만난다(추정, 에디터 확인 전). 후보: `head gate slack` 재튠(rest 깊이 머리로 튠한 값 — "driver가 들어오면 gate 여유를 다시 본다") / `neck mid` gate(V넥 (3)) / 머리 복원이 높이를 바꾸는 것 자체 `[N29]` |
