...
- **자기겹침 지도 — 전부 닫혔다.** 2026-09-07, 네 보정이 모두 든 케이지로 잰 스윕(§7b, 손을 rest에 묶고 seed 1):
    ...
    | **F 발등이 발볼을 지난다** (열림, 2026-09-12) | `L/R ankle+L/R toe` 1·1 · `L/R tip+L/R toe` 1·1 (3 삼각형씩) | 고관절 1.5 **+** 발 뼈 0.5 (각각 단독은 무사) | 룰 8이 ankle 링의 앞 여유를 45° 기울어진 d를 따라 ×1.5 하면 발등 코너가 Foot 앞 7.84 cm에 서는데, 발 뼈 0.5의 toe 링 평면은 Foot 앞 6.58 cm다 — 정강이–발등 판이 toe 링을, 발등–발끝 뚜껑이 tip을 뚫는다. ankle의 "깊이"가 앞뒤와 위아래를 함께 담은 축이라 복원이 발등을 앞으로도 민다. 후보: gate `ankle behind toe`(앞변을 toe 평면 뒤로, 링째) / ankle 자리를 앞 성분에 대해 따로 / 발 뼈 0.5를 범위 밖으로 `[N29]` |
