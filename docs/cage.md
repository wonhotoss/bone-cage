...
| 옆판(wall) | 제어점 사슬. 이웃 쌍마다 쿼드 1(hi–hi–lo–lo). 대각선은 오른쪽 면에서 반대 `[N3]`; 발 안쪽 ankle–toe 쿼드 둘만 §5b `turned`가 돌린다 `[N31]`. | `topology`, `turned` |
| 여유(reach) | 잰 구간 바깥으로 더하는 상수. 링: `front` `back`(깊이 축, **hi/lo 변별**), `hi`/`lo`(±s 쪽), `outward hi`/`outward lo`(변별 n 방향 이동). 음수 = 안쪽. 모두 씬 단위. | `recipe` |
| 복원 / 구간(section) | 걸침 뒤, 보정 앞에서 도는 단계. **구간** = 깊이를 자기 폭에서 되찾는 링 하나(몸통 셋·arm 둘·elbow·knee·ankle·toe·crown·head, 또는 기둥 쌍인 고관절 링·발끝 캡)와, 그 위에 서서 닫는 코너들의 깊이 평균을 받는 기둥. `by = 폭 / rest 폭`을 d 여유에 곱한다 — 여유는 각자 걸린 관절(**자리**: 몸통은 Hips, 사지는 자기 관절, 발은 셋 다 Foot) 기준이고, 결과는 rest의 깊이/폭 비 복원. §6 복원 `[N29]` | `cage_section`, `cage_post.hi_mean`, `restore` |
...
| `valley_reach` | 0.011 (튠 §7) | 씬 | 손가락 계곡 제어점을 손목 반대 방향으로 미는 거리. `cage_tune`의 값 |
...
열: **앵커** = 링을 놓는 관절(변별로 분리됨, §6). **감쌀 살** = 서브트리 루트. **종류** cap/joint/split = 측정 창(§1). **hi/lo**가 `A → B 사이`면 그 변은 잰 여유 대신 두 제어점 `A`·`B`를 잇는 선분 위에 놓인다(§6 걸친 변, `[N21]`). 행이 **네 귀퉁이 걸침**이면 네 코너가 각자의 선분과 링 평면의 교점이고 잰 값·여유는 전부 무시된다(§6, `[N27]`). `↷θ` = `side` 축으로 θ만큼 앞으로 기울인 축(`n = cos·up + sin·depth`, `d = cos·depth − sin·up`). 여유는 씬 단위, 빈칸 = 0. `front`/`back`은 hi/lo 변별 — 한 값이면 양 변 공통, `a / b`면 hi 변 / lo 변, **`body`면 몸통 공유 깊이**(§3f: d 앵커 = Hips, 여유 = `body front`/`body back`) `[N28]` — rest에서의 공유이고, 변형 뒤에는 §6 복원이 링마다 자기 폭에서 깊이를 되찾는다. 사지 링(elbow·knee·ankle·toe)의 깊이도 같다 — 자기 관절 기준, 발은 셋 다 Foot 기준 `[N29]`. **girth** = 실루엣(`s_hi`·`s_lo`·`along_hi`·`along_lo`)을 곱하는 span(§6): 뼈면 그 끝 관절의 이름 — 사지의 링은 전부 그 사지의 뿌리 뼈(쇄골 / 고관절)를 적는다 `[N22]` — 머리 링 둘은 `키`(§1) `[N23]`; 빈칸 = 곱하지 않음.

| 이름 | 앵커 | 감쌀 살 | n | s | d | 종류 | front | back | hi | lo | outward hi | outward lo | girth | 비고 |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `crown` | Head | Head | +up | side | depth | cap | 0 | 0 | | | | | 키 | 정수리 캡. 폭·높이가 키를 따른다 `[N23]`, 깊이는 복원(§6)이 그 폭에서 — 자리 Head `[N29]`. `front`·`back`(0.01 / 0.004, 튠 §7) — 가슴~배꼽 정중선과 날개뼈가 몸통 판을 뚫음 |
| `head` | Head | Head | up↷25° | side | depth↷25° | split | 0 | 0 | | | 0.023 | 0.023 | 키 | 머리–목 분리 평면. 폭과 오프셋이 키를 따른다 `[N23]`, 깊이는 복원(§6) — 자리 Head `[N29]`. 기울기·오프셋은 씬의 head splitter에서 읽은 값; 기울기 25° · 오프셋 0.023 · `front` 0.01 · `back` −0.04는 튠 값(§7) `[N12]` |
| `L arm` | LeftArm | LeftShoulder | +side | up | depth | joint | body | body | — | — | — | — | LeftArm | 몸통 판과 어깨 판의 경계 `[N2]`. 실루엣 변은 재지 않는다 — 정면에서 Arm 관절을 지나는 **라글란 이음선**의 양끝: `arm tilt`(위끝이 안쪽으로, 15°) · `arm length`(rest 0.18), 관절이 중점 `[N11]`. 이음선 길이는 **쇄골**(Shoulder→Arm, 이 링의 앵커 뼈)의 현재/rest 길이 비를 따른다 — `girth` = LeftArm(§6). 상완 판은 여기서 elbow 링으로 바로 간다(§5a). 깊이는 rest에서 몸통 공유(§3f) `[N28]`, 변형 뒤엔 복원(§6) `[N29]` |
| `L elbow` | LeftForeArm | LeftArm | +side | up | depth | joint | | | 0.01 (튠 §7) | | | | LeftArm | |
| `L wrist` | LeftHand | LeftHand | +side | up | depth | joint | | | | | | | LeftArm | 단면은 손이 덮어씀 §4a |
| `R arm` | RightArm | RightShoulder | −side | up | depth | joint | body | body | — | — | — | — | RightArm | `[N2]`. L arm과 같은 이음선 |
| `R elbow` | RightForeArm | RightArm | −side | up | depth | joint | | | 0.01 (튠 §7) | | | | RightArm | L elbow와 공통 |
| `R wrist` | RightHand | RightHand | −side | up | depth | joint | | | | | | | RightArm | §4a |
| `spine` | Spine | Hips | +up | side | depth | joint | body | body | `L hip` → `L arm`·lo 사이 | `R hip` → `R arm`·lo 사이 | | |  | 몸통 판의 아랫변, 허리. 아래는 골반 기둥 §3c `[N13]`. pelvis를 줄이면 링이 고관절 기둥 아래로 내려가는데, 그것은 §6b `spine above hips`가 링째 멈춰 막는다 `[N16]`. 깊이는 rest에서 몸통 공유(§3f) `[N28]`, 변형 뒤엔 복원(§6) `[N29]` |
| `spine1` | Spine1 | Hips | +up | side | depth | joint | — | — | 네 귀퉁이 걸침: 코너 c = 그쪽 `arm`·lo 코너 c → `spine` 코너 c 사이 | 〃 | | |  | 배. 몸통 판의 가로대 하나 `[N10]`. 겨드랑이에서 spine 링으로 내려가는 네 직선 위의 중간 링 — 자기 폭·깊이가 없다 `[N27]`; 깊이는 복원(§6)이 그 폭에서 되찾는다 `[N29]` |
| `spine2` | Spine2 | Hips | +up | side | depth | joint | — | — | 〃 | 〃 | | |  | 아랫가슴. 〃 |
| `L knee` | LeftLeg | LeftUpLeg | −up | side | depth | joint | | 0.02 (튠 §7) | 0.005 (튠 §7) | | | | LeftUpLeg | 자기 다리 살만 잰다 `[N13]`. `hi` = 바깥쪽 변 |
| `L ankle` | LeftFoot | LeftLeg | −up↷45° | side | depth↷45° | joint | −0.08 (튠 §7) | 0.005 (튠 §7) | | | | | LeftUpLeg | 발목. Foot 관절을 지나 뒤로 기울어진 링 — 뒤꿈치에서 발등–정강이 연결부로. 기울기 45°·`front`(발등 쪽)·`back`(뒤꿈치 쪽)은 튠 값(§7) `[N14]`. 복원(§6)의 자리는 Foot — 뒤꿈치 코너가 발바닥과 같은 양만큼 내려간다 `[N29]` |
| `L toe` | LeftToeBase | LeftFoot | +depth | side | up | joint | | (바닥) | | | | | LeftUpLeg | 발볼. 발 방향에 직교하는 세로 링, front = 발등, back = 발바닥. **d 앵커는 위·아래 모두 Foot**: 아래 여유 = ankle 링 바닥 높이(`floor`) — 발바닥이 뒤꿈치와 수평 `[N14]`; 위 여유 = 잰 발등 높이 − Foot — 발 뼈가 길어져 ToeBase가 내려가도 단면은 그대로 `[N30]`. 복원(§6)의 자리 Foot — 바닥이 뒤꿈치와 함께 내려가 수평이 남고 발바닥 두께가 스케일을 따른다 `[N29]` |
| `R knee` | RightLeg | RightUpLeg | −up | side | depth | joint | | 0.02 (튠 §7) | | 0.005 (튠 §7) | | | RightUpLeg | `s = side`라 `lo`가 바깥쪽 변; 여유는 L knee와 공통 |
...
ankle의 `↷`는 `n`의 기준이 `−up`이라 `n = −cos·up + sin·depth`, `d = cos·depth + sin·up` — knee 프레임을 `side` 축으로 돌려 toe 프레임 쪽으로 가는 도중이다. 발끝은 링이 아니라 기둥 §3d. **평평한 발바닥, 그리고 발의 높이는 전부 Foot에서**: `floor = Foot·up − (ankle 링 rest lo_back 코너)·up`. toe 링은 d 앵커 둘 다 Foot — `hi_back = lo_back = floor`, `hi_front = lo_front = 잰 위 − Foot·up`(ToeBase 기준 측정값에서 `Foot − ToeBase`의 `up` 낙차를 뺀 것); tip 기둥의 양끝도 같다(§3d) `[N14]` `[N30]`. 위쪽 값은 살에서 잰다.
...
코너별 깊이 `{hi,lo}_front = hi_d − max(앵커·d) + front.{hi,lo}`, `{hi,lo}_back = min(앵커·d) − lo_d + back.{hi,lo}`. d 앵커(`d_hi_anchor`/`d_lo_anchor`) = 앵커 전체; toe 링만 뒤(바닥)의 d 앵커를 Foot으로 덮어쓴다(§3d). `body` 링은 이 깊이 규칙을 쓰지 않는다 — d 앵커 = Hips, 네 코너 여유 = `body front`/`body back`(§3f).
...
앞/뒤 판의 가로대가 정중선을 지나는 자리마다 기둥 하나. 몸통·머리·골반 판을 좌/우 반판으로 가른다 `[N10]`. **띠** = 기둥이 닫는 가로대의 링. 기둥은 두 종류다 — **띠의 깊이를 받는 기둥**(`crown`·`head`·`spine` mid): `d` 앵커와 깊이 여유가 그 링의 것이라 앞/뒤 정점이 링의 변과 같은 깊이에 놓인다(hi/lo 코너 여유의 평균). **몸통 깊이 기둥**(`neck mid`): d 앵커 = Hips, 여유 = `body front`/`body back`(§3f) — arm 링과 같은 선 위 `[N28]`. **교점 기둥**(`sternum`·`spine1`·`spine2` mid): 자기 깊이가 없고, 앞끝 = 두 제어점을 잇는 선분과 정중선 평면의 교점, 뒤끝도 같다(§6 걸친 끝, `[N27]`). 그렇게 놓인 뒤 **복원**(§6)이 정중선 기둥 일곱 전부(`crown`·`head`·`neck`·`sternum`·`spine`·`spine1`·`spine2` mid)의 깊이를 **닫는 코너들의 평균**으로 다시 정한다 — 링이 자기 폭에서 깊이를 되찾으면 그 위의 기둥이 따라와야 하기 때문이다. rest에서는 어느 규칙으로 놓아도 같은 자리다 `[N29]`.

| 이름 | 띠 | 앵커(가중치) | 판 내 위치 | 비고 |
|---|---|---|---|---|
| `crown mid` | crown | Head (1) | crown 앞변 중점 | 복원 뒤 깊이는 crown 링 앞변·뒷변 평균(§6) `[N29]` |
| `head mid` | head | Head (1) | head 앞변 중점 | `d`는 head 링의 기울어진 `d`. 복원 뒤 깊이는 head 링 앞변·뒷변 평균(§6) `[N29]` |
| `neck mid` | L·R arm | Neck (1) | Neck | V넥 바닥. 두 arm 링의 hi 변과 함께 V를 이룬다 `[N11]`. 깊이 = 몸통 공유(§3f) `[N28]`, 복원 뒤 L·R arm hi 변 코너 평균(§6) `[N29]` |
| `sternum mid` | L·R arm | Spine3 (1) | 교점: 앞 = `L arm`·lo_front → `R arm`·lo_front, 뒤 = lo_back 둘 | 겨드랑이 선의 정중선 교점. 높이·깊이가 전부 겨드랑이에서 오고 Spine3은 정중선 평면만 준다 — 가슴 띠가 구성상 평면 `[N27]`. 복원 뒤 깊이는 같은 코너 넷의 평균(§6) `[N29]` |
| `spine2 mid` | spine2 | Spine2 (1) | 교점: 앞 = spine2 hi_front → lo_front, 뒤 = hi_back → lo_back | `[N27]`. 복원 뒤 깊이는 자기 링 앞변·뒷변 평균(§6) `[N29]` |
| `spine1 mid` | spine1 | Spine1 (1) | 교점: 앞 = spine1 hi_front → lo_front, 뒤 = hi_back → lo_back | `[N27]`. 〃 |
| `spine mid` | spine | Spine (1) | spine 앞변 중점 | 링 위 정중선 사슬의 끝. 아래로는 골반 반판의 세로 가로대 `spine mid – crotch`. 복원 뒤 깊이는 spine 링 앞변·뒷변 평균(§6) `[N29]` |

**bake 규칙**: 링 위의 기둥(`midline`) — 판 내 위치 = rest 앞변 중점, 앵커 관절과의 차를 오프셋으로 굽는다. `neck mid`(`post`) — 판 내 위치 = Neck 그 자리(오프셋 0), d 앵커 = Hips, 여유 = `body back`/`body front`(§3f). 교점 기둥(`crossing`) — 앵커 관절, 오프셋 0, `d` 여유 0; `hi_between`·`lo_between`에 두 정점, `between_axis = side`. 구운 상수가 없다. 복원의 `hi_mean`·`lo_mean`(§6 표)은 정중선 기둥 일곱과 `crotch`에 함께 적는다 `[N29]`.
...
| 상수 | 값 | 단위 | 의미 |
|---|---|---|---|
| `crotch drop` | 0.08 (튠 §7) | 씬 | Hips 관절에서 crotch까지 `−up` 거리 |
| `hip out` | 0.8 (튠 §7) | 비율 | 바깥 고관절 점이 UpLeg에서 더 나가는 crotch→UpLeg 거리의 배수 |

**깊이(bake 규칙)**: 세 기둥이 **몸통 깊이를 공유**한다(§3f) — `d` 앵커는 셋 다 **Hips**(양끝), 여유 = `body back`/`body front`. 그래서 골반 판은 허리 링과 허벅지 사이에서 평평한 판이고 spine 링과 같은 앞뒤 선 위에 있어, 고관절 편집에도 앞뒤가 흔들리지 않는다 `[N28]`. **복원(§6) 뒤에는** `L/R hip`이 자기 고관절 링(`crotch` ↔ 그 hip)의 폭에서 깊이를 되찾고 `crotch`는 둘의 평균이라, spine 링과 한 선인 것은 rest에서다 `[N29]`. 판 내 오프셋: `crotch`는 `−up·drop`, `L/R hip`은 `+up·f·drop`(아핀 결합 `(1+f)·UpLeg − f·Hips`에 crotch의 drop을 f배 더한 것 = `UpLeg + f·(UpLeg − crotch)`).
...
|---|---|---|---|---|---|
| `L tip` | L tip·hi / ·lo | ToeBase, Foot (1+f, −f) | 가상 endbone에서 `side`로 발가락 살 폭(`wide_hi` / `wide_lo`)까지 | 위: **Foot**, 발가락 살 `up` 구간(inflate)의 위끝 − Foot `[N30]`. 아래: **Foot**, `floor`(§3) — toe 링 바닥과 같은 높이 | LeftUpLeg |
| `R tip` | R tip·hi / ·lo | 〃 | 〃 | 〃 | RightUpLeg |

**bake 규칙**: 발가락 살 = ToeBase 서브트리의 살. `f = max(살·dir[ToeBase] − ToeBase) · (1+margin) / rest_len(ToeBase)` — 발가락이 ToeBase 너머로 뻗은 길이의 발 뼈 길이 비율. 그래서 발 길이를 늘이면 발끝이 비례해 따라 나간다. 같은 함수가 toe 링의 바닥을 Foot에 건다(§3 평평한 발바닥). 판 내 위치(`reach`, 발가락 폭)는 링의 girth와 같은 규칙으로 고관절 뼈의 비를 곱한다(§6) — 뚜껑이 자기가 닫는 toe 링과 같은 폭으로 남는다 `[N22]`. 두 기둥은 하나의 구간(§6 복원)이다: 폭 = 두 기둥 사이, 양끝이 다 Foot에 걸려 있으니 자리도 Foot — 발바닥과 함께 내려가고 위끝은 그만큼 오른다 `[N29]`.

### 3e. 어깨 기둥 — 없음

어깨에는 기둥이 없다. 상완 판은 arm 링의 두 변에서 elbow 링의 두 변으로 곧게 간다(§5a). 삼각근 위에 세웠던 `delt` 기둥은 2026-09-09에 지웠다 `[N15]`.

### 3f. 몸통 깊이 — `body front` / `body back`

사지와 머리를 뺀 몸통 전체가 **앞뒤 선 한 쌍**을 공유한다: 옆에서 보면 몸통은 상자다 `[N28]` — **rest에서**. 변형 뒤에는 복원(§6)이 구간마다 깊이를 자기 폭에서 되찾아, 상자는 링마다 다른 깊이로 갈라진다 `[N29]`.

| 항목 | 정의 |
|---|---|
| 몸통 살 | Hips 서브트리의 살에서 LeftUpLeg·RightUpLeg·LeftArm·RightArm·Head 서브트리를 뺀 것 (골반·척추·목·양 Shoulder) |
| `body front` / `body back` | 몸통 살의 `depth` 구간을 inflate한 앞끝/뒤끝 − Hips 좌표 + 튠 `body front`/`body back`(씬, §7, 0 / 0). d 앵커는 항상 **Hips** |
| 공유하는 것 | `L/R arm` 링 네 코너, `neck mid`, `spine` 링 — 그리고 절두체·교점으로 그 사이에 놓이는 `spine1`·`spine2`와 mid 셋, `sternum mid`(§6) — `crotch`·`L/R hip` |
| 공유하지 않는 것 | `crown`·`head` 링과 그 mid(자기 살, `crown/head front/back`), 사지의 모든 링과 기둥 |
...
| 손목 링 덮어쓰기 | 실루엣 축(`up`): hi = 판 위, lo = 판 아래 + `wrist_drop`. 깊이 축(`depth`): 손목 평면에서 `rest_len(Middle1)·0.5` 이내 살의 구간을 inflate + 여유 `wrist thumb`(엄지 쪽, −0.016) / `wrist pinky`(새끼 쪽, −0.004), 튠 §7 `[N5]`. **`girth_d` = Thumb2 → Pinky1 span(`s` 축)** — 손바닥 폭, 첫 마디가 길어지며 벌어지는 만큼 `[N25]` |
...
| 이름 | 앵커(가중치) | 오프셋 |
|---|---|---|
| `L thumb out` | Thumb2 (1) | `s`로 `wide_hi`까지 + `thumb out`(−0.01, 튠 §7) |
| `L thumb\|index` | Thumb2, Index1 (½, ½) | 손목→중점 방향(판 내 투영)으로 `valley_reach`(0.011, 튠 §7) |
| `L index\|middle` | Index1, Middle1 (½, ½) | 〃 |
| `L middle\|ring` | Middle1, Ring1 (½, ½) | 〃 |
| `L ring\|pinky` | Ring1, Pinky1 (½, ½) | 〃 |
| `L pinky out` | Pinky1 (1) | `s`로 `wide_lo`까지 + `pinky out`(−0.01, 튠 §7, −s 쪽) |
...
- **반경**: 관절 `J` 서브트리 살의 `perp` 구간을 inflate. 기둥 오프셋 = `perp · (구간 끝 − 관절의 perp 좌표 ± (finger_reach + finger out))` — `finger out`은 모든 손가락 링 양변에 같은 여유를 더하는 knob(0.0005, 튠 §7).
...
씬 단위, 좌우 손 공통. 표에 없는 링은 잰 값 그대로. 위에 `finger out`(0.0005, 튠 §7)이 모든 링 양변에 일괄로 더해진다 — 표는 링 하나의 예외, knob은 손가락 전체의 바닥 여유.
...
**대각선을 돌린 쿼드** — `turned`. 옆판 쿼드의 대각선은 `strip`이 사슬 방향으로 긋는다(도착 기둥의 앞 ↔ 출발 기둥의 뒤, §5d). 아래 쿼드만 반대로 긋는다 — 사슬 순서로 두 기둥을 적는다 `[N31]`.

| 사슬 | 쿼드 (출발 → 도착) | 대각선 |
|---|---|---|
| 2 | L toe·lo → L ankle·lo (왼발 안쪽) | L toe 앞(v51) – L ankle 뒤(v46): 바깥쪽(v48–v45)과 같은 방향. 돌리기 전은 발등(v47)–발바닥(v50) |
| 2 | R ankle·hi → R toe·hi (오른발 안쪽) | R toe 앞(v60) – R ankle 뒤(v57) |
...
- **표가 선언하는 것은 고리와 사슬뿐이다**(목 반판 둘의 시작점이 예외로 대각선까지 정한다, `[N3]`)**.** 판을 채우는 ladder의 가로대(`v[i]`–`v[j]` 짝)와 쿼드를 가르는 대각선은 `strip`의 선택이며 어느 행도 이름하지 않는다 — §5b `turned`의 쿼드 둘이 유일한 예외다 `[N31]`. 그래서 사슬의 연속 기둥 쌍 — 그것이 곧 껍질의 링이고, 기울어진 고관절 링(`crotch` 옆 `L/R hip`)이 여기서 나온다 — 을 `cage_constants.grid`에 정점 쌍으로 남긴다. 삼각화는 이 구분을 지우므로 `tris`에서 되돌릴 수 없다. 디버그 와이어(§7)가 읽는다.
...
깊이는 d 앵커의 구간에 코너별 여유: `front = d·(max(d_hi앵커·d) + c_front·g_d)`, `back = d·(min(d_lo앵커·d) − c_back·g_d)` (c = hi, lo 변; d 앵커는 보통 양 변 앵커 전체). `g_d`는 링의 **`girth_d` span**의 비 — 깊이가 무엇을 따르는지를 실루엣과 따로 선언한다. 없으면 `g_d = 1`. 지금은 손목 링만 갖는다(§4a, Thumb2 → Pinky1) `[N25]`. 깊이가 **선언된 span**을 따라야 할 때 적는 열이다; 몸통의 깊이는 span이 아니라 **복원**(아래)이 자기 폭에서 되찾는다 `[N29]`.
...
**복원** (`restore`): 걸침 뒤, 보정(§6b) 앞. 레시피가 변형한 변(실루엣)을 기준으로 변형하지 않은 변(깊이)을 고쳐 **rest의 깊이/폭 비를 되찾는다** `[N29]`. 두 단계, 뒤가 앞을 읽는다. 깊이 축만 만지고 보정 넷은 `up`·`side`만 만지므로 둘은 서로 읽는 것이 없다.

1. **구간**(`cage_section`, 표 순서대로): 폭 = `hi` 정점 무리의 중심 ↔ `lo` 정점 무리의 중심 거리 중 `d`에 직교하는 성분. `by = 폭 / rest 폭`(`rest_width`는 bake가 복원 없이 놓은 rest 케이지에서 굽는다). `front` 정점의 `d` 좌표를 `front_seat` 관절의 `d` 좌표 기준으로, `back` 정점을 `back_seat` 기준으로 `by`배 — 자리는 그 여유가 걸린 d 앵커 그 관절이라, 링 식의 `g_d`를 span 대신 **자기 폭의 비**로 쓰는 것과 정확히 같다. 깊이 여유는 레시피가 건드리지 않으므로(`girth_d` 없음) 결과는 rest의 깊이/폭 비 복원이다. 두 자리는 지금 모든 구간에서 같은 관절이다(몸통 Hips, 사지 링 자기 관절, 발 Foot). rest에서 `by = 1`. 구간마다 독립이다: 몸통 공유 깊이(§3f)와 절두체(§6 2)는 놓을 때의 규칙이고, 복원은 링마다 자기 폭만 본다.

| 이름 | hi | lo | front | back | seat | d |
|---|---|---|---|---|---|---|
| `spine` | spine hi 변 (코너 2) | spine lo 변 (코너 2) | spine hi_front·lo_front | spine hi_back·lo_back | Hips | depth |
| `spine1` | 〃 | 〃 | 〃 | 〃 | Hips | depth |
| `spine2` | 〃 | 〃 | 〃 | 〃 | Hips | depth |
| `L arm` | L arm hi 변 (승모근) | L arm lo 변 (겨드랑이) | L arm hi_front·lo_front | L arm hi_back·lo_back | Hips | depth |
| `R arm` | 〃 | 〃 | 〃 | 〃 | Hips | depth |
| `L hip` | `L hip` 두 끝 | `crotch` 두 끝 | `L hip` 앞끝 | `L hip` 뒤끝 | Hips | depth |
| `R hip` | `R hip` 두 끝 | `crotch` 두 끝 | `R hip` 앞끝 | `R hip` 뒤끝 | Hips | depth |
| `L elbow` / `R elbow` | 그 링 hi 변 | lo 변 | hi_front·lo_front | hi_back·lo_back | 자기 관절 (L/RForeArm) | depth |
| `L knee` / `R knee` | 〃 | 〃 | 〃 | 〃 | 자기 관절 (L/RLeg) | depth |
| `L ankle` / `R ankle` | 〃 | 〃 | 〃 (발등 쪽) | 〃 (뒤꿈치) | L/RFoot | depth↷45° |
| `L toe` / `R toe` | 〃 | 〃 | 〃 (발등) | 〃 (발바닥) | L/RFoot (§3: 위·아래 d 앵커가 다 Foot `[N30]`) | up |
| `L tip` / `R tip` | `L tip`·hi 두 끝 | `L tip`·lo 두 끝 | 두 기둥의 위끝 | 두 기둥의 아래끝 | **L/RFoot** | up |
| `crown` | crown hi 변 | lo 변 | hi_front·lo_front | hi_back·lo_back | Head | depth |
| `head` | head hi 변 | lo 변 | 〃 (턱 쪽) | 〃 (뒤통수) | Head | depth↷25° |

arm 링의 폭은 이음선 길이 그 자체(hi·lo 변이 `n`으로도 벌어져 있어 `s` 성분만이 아니다), spine 셋의 폭은 걸친 변·절두체가 준 것, `L/R hip`의 폭은 기울어진 고관절 링의 빗변. 몸통 일곱의 자리는 Hips — 몸통 상자의 자리(§3f)라 rest의 `body front`/`back` 여유가 그대로 `by`배 된다. 사지 링의 폭은 뿌리 뼈의 비(`girth`)가 준 것이라 결과는 `girth_d = girth`와 같고, 자리는 자기 관절. **발(ankle·toe·tip)의 자리는 셋 다 Foot이다** — 발의 높이가 전부 Foot에서 재어지기 때문이다(§3 `[N30]`): 뒤꿈치 코너·toe 링 바닥·tip 아래끝이 같은 양 `(by−1)·floor`만큼 함께 내려가 발바닥이 수평으로 남고, 발바닥 두께가 스케일을 따라 커지며, 발등·발가락 위(Foot +0.5 / −0.03 cm)는 거의 제자리다 — 성장은 아래로 간다. **머리 링 둘의 자리는 Head**: 폭이 키를 따르므로(`[N23]`) 깊이도 키를 따르게 된다 — 턱·뒤통수가 Head 관절 기준으로 벌어진다. 손목 링과 손은 복원하지 않는다 — 세 축이 이미 선언되어 있다 `[N25]`.

2. **기둥 평균**(`cage_post.hi_mean`·`lo_mean`): 끝의 `d` 좌표 = 적힌 정점들의 `d` 평균. 판 내 위치는 그대로. 두 구간 사이의 기둥(`neck`·`sternum`·`crotch`)은 양쪽의 평균이고, 한 링 위의 기둥은 그 링의 변 위다.

| 기둥 | 앞끝 ← | 뒤끝 ← |
|---|---|---|
| `neck mid` | L·R arm hi_front | L·R arm hi_back |
| `sternum mid` | L·R arm lo_front | L·R arm lo_back |
| `spine mid` | spine hi_front·lo_front | spine hi_back·lo_back |
| `spine1 mid` | spine1 〃 | spine1 〃 |
| `spine2 mid` | spine2 〃 | spine2 〃 |
| `crown mid` | crown hi_front·lo_front | crown hi_back·lo_back |
| `head mid` | head 〃 | head 〃 |
| `crotch` | `L hip`·`R hip` 앞끝 | `L hip`·`R hip` 뒤끝 |

### 6b. 보정 — `control_points`의 마지막 단계

링과 기둥과 걸침과 복원이 **전부 끝난 뒤** 도는 단계. 평가 중 제어점이 서로를 읽는 것은 걸침과 복원(§6) 둘뿐이고 그것은 위치이지 판단이 아니다 — 케이지의 한 부분을 다른 부분에 비추어 **판단**하는 일은 전부 여기로 온다 `[N20]`. **표의 순서가 우선순위다**: 뒤의 보정이 앞의 보정이 옮긴 정점을 다시 옮길 수 있다.
...
|---|---|---|---|---|---|---|
| `L arm above spine2` / `R arm above spine2` | 그쪽 arm 링 전체 (정점 4) | 그 링의 lo 변 — 겨드랑이 (정점 2) | spine2 링의 그쪽 코너 둘 | −0.01 (튠 §7) | `up` | 어깨 밑동·upper thorax를 줄이고 쇄골을 늘이면 겨드랑이(이음선 절반 아래)가 Spine2까지 내려온다. 절두체의 교점이 끝에 붙어 spine2 링이 겨드랑이 코너와 한 자리가 되고, 복원이 둘의 깊이를 따로 놓으면 그 사이 띠가 접혀 몸통 반판·상완 판을 뚫는다(§9 지도 H). 재는 곳은 겨드랑이 변, 세우는 것은 **링 전체**라 이음선 기울기가 남는다. **여유는 음수여야 한다** — clamp된 상태에서 바닥은 겨드랑이의 옛 자리 그 자체라, 여유가 곧 벌어지는 띠의 높이다. `head above arms`보다 먼저 돈다(그것이 읽는 어깨를 옮기므로) `[N20]` |
| `neck mid above sternum` | `neck mid` 두 끝 (정점 2) | 그 둘 | `sternum mid` 두 끝 | −0.02 (튠 §7) | `up` | chest를 줄이면 Neck 관절과 함께 V 바닥이 겨드랑이 선으로 내려오고, 어깨 밑동이 길고 쇄골이 짧으면 겨드랑이와 그 교점(`sternum`)이 올라온다 — sternum이 V 바닥 위에 서면 몸통 꼭대기 가로대 둘이 순서를 바꿔 목 판이 가슴 띠를 뚫는다(§9 지도 G′ V 바닥). 기둥만 올린다(이음선 변은 그대로). **여유는 음수** — V의 높이를 요구한다. rest 차 12.6 cm `[N20]` |
| `head above arms` | crown 링, head 링, `crown mid`, `head mid` (정점 12) | head 링 네 코너 중 최하단 | L·R arm 링의 hi 변 (정점 4) | **0.01** (튠 §7 — 2026-09-12; 처음 0.038, 씬 0.03이었다) | `up` | 목을 줄이면 파팅 평면이 이음선 아래로 내려가 목 판이 머리를 뚫는다 — 머리 뭉치를 어깨 위로 되올린다 `[N20]` |
| `L arm beside head` / `R arm beside head` | 그쪽 arm 링 전체 (정점 4) | 그 링의 hi 변 (정점 2) | head 링 네 코너 중 그쪽으로 가장 바깥 | **−0.03** (튠 §7 — 2026-09-13; 처음 0, 씬 −0.01이었다) | `+side` / `−side` | 쇄골을 줄이면 arm 링이 관절을 따라 정중선 쪽으로 오다 hi 변(승모근 점)이 머리 실루엣 안에 선다 — 옆벽이 안쪽으로 눕고 목 반판이 머리 반판을 뚫는다. 재는 곳은 hi 변, 세우는 것은 **링 전체**라 라글란 기울기가 남는다. head gate와 축이 직교해 순서 무관 `[N20]` |
| `spine above hips` | spine 링, `spine mid` (정점 6) | 그 여섯 중 최하단 | L·R `hip` 기둥의 네 끝 | −0.005 (튠 §7) | `up` | pelvis를 줄이면 spine 링이 고관절 기둥 아래로 내려가고 골반 판이 위로 접혀 몸통 판을 뚫는다 — 링과 그 정중선 기둥을 함께 멈춰 가로일자를 유지한다. 옆변만 붙들면 정중선이 계속 내려가 V가 열리고, 고관절 기둥까지 안으로 걸어 들어오면 허벅지 판이 그 V를 가로지른다 `[N16]`. **여유는 음수여야 한다** — 고관절 기둥과 같은 높이에 서면 그 사이 골반 판이 납작해져 몸통 판과 스친다 `[N20]` |
| `L knee beside crotch` / `R knee beside crotch` | 그쪽 knee 링 전체 (정점 4) | 그 링의 안쪽 변 (정점 2; `L knee`는 `lo`, `R knee`는 `hi`) | `crotch` 기둥 두 끝 | −0.005 (튠 §7; 처음 −0.01) | `+side` / `−side` | 고관절 뼈가 순수 측방이라 그 길이가 골반 반폭이다 — 줄이면 다리가 링째 안으로 걸어 들어와 무릎 링 안쪽 변이 정중선을 넘고 두 다리의 안쪽 벽이 서로를 통과한다. 재는 곳은 안쪽 변, 세우는 것은 **링 전체**라 허벅지가 얇아지지 않는다. 바닥이 어느 보정도 옮기지 않는 `crotch`라 좌우가 서로를 읽지 않는다. **여유는 음수여야 한다** — 0이면 두 링이 정중선에 겹쳐 서고 그 사이 판이 여전히 스친다 `[N20]`. 발목은 별도 보정이 필요 없다 |

rest에서는 머리가 이미 어깨 위에 있고, arm 링 hi 변도 머리 폭 바깥(Arm 관절 side 0.16 대 머리 반폭 약 0.09), spine 링 바닥도 고관절 기둥보다 3.7 cm 위, 무릎 링 안쪽 변도 가랑이에서 3.98 cm 떨어져 있고, 겨드랑이(arm 링 lo 변, up 40.6)도 Spine2(≈30.1) 위 10.5 cm라 **다섯 보정**(거울쌍 셋이 각각 두 줄이라 선언은 여덟) 모두 `lift = 0`이므로 **rest 케이지는 보정되지 않는다**(분기 없이 성립).

## 7. 검증·디버그 — `mapping_tester`

| 기능 | 동작 |
|---|---|
| 슬라이더 | 편집 대상 뼈 53개(몸통 23 + 손가락 2×5×3), 값은 **rest 대비 비**(0.5~1.5; 스윕의 케이스 이름과 같은 단위, 2026-09-13부터 — 그 전엔 절대 길이). 변경마다 `update_body()` = 케이지 재생성 → deform → rest pose 재바인딩. |
...
| `rebuild cage` | 재bake + 케이지 갱신. 이 문서의 상수를 바꾸면 누른다. bind는 하지 않는다 — 아래 `bind mesh`. |
| `bind mesh` | rest 메시를 현재 rest 케이지에 재bind(수 초) + `update_body`. 케이지가 bind 때와 달라지면(튠 슬라이더·`rebuild cage`·좌표 방식 변경) 그 위에 경고 "mesh is not bound to the current cage"가 뜬다(씬을 새로 열어 bind가 없을 때도) — 판정은 저장된 플래그가 아니라 bind가 풀린 rest 케이지와 현재 상수가 만드는 rest 케이지의 비교다. 누르지 않은 채 뼈 슬라이더·`check containment`·export를 쓰면 `mapped`가 그때 한 번 bind한다(2026-09-14). |
...
| 튠 슬라이더 (`cage_tune`) | §3·§6b의 튠 값을 인스펙터에서 찾는 편집기. **값은 코드에만 있다** — `cage_tune.baked`가 확정값이고(2026-09-14, 네 층 스윕 0 뒤), 씬(`mapping_tester.tune_delta`)에는 거기서 벗어난 **diff**만 남는다(기본 전부 0). bake는 `cage_tune.tuned(delta)` = baked + delta를 읽으므로, diff 0인 새 tester도 헤드리스 bake도 `main.unity`의 케이지를 만든다. 슬라이더는 diff를 움직이고(0 = 확정값) 라벨이 결과 절대값을 찍는다; 범위는 아래 절대 범위를 확정값만큼 옮긴 것. `reset tune`이 diff를 0으로 돌린다. 비율이 아니라 덧셈인 것은 값 상당수가 0이거나 음수라서다. 현재(값, 범위): arm 링 `tilt`(라글란 이음선의 기울기, 15°, −30..45) · `length`(이음선 길이, 0.18, 0.05..0.3), 몸통 공유 깊이 `body front`·`body back`(0 / 0, −0.05..0.1; §3f `[N28]`), head 링 `tilt`(25°, 0..45) · `offset`(0.023, −0.02..0.06) · `front`·`back`(0.01 / −0.04, −0.05..0.1), crown 링 `front`·`back`(0.01 / 0.004, −0.05..0.1), gate 여유 여섯(§6b; 음수는 간격 요구): `head gate slack`(머리 보정, 0.01, 0..0.1) · `arm gate slack`(어깨 보정: arm hi 변이 머리 폭 안쪽으로 들어와도 봐주는 거리, −0.03, −0.05..0.05) · `spine gate slack`(허리 보정: spine 링 바닥이 고관절 기둥 아래로 내려가도 봐주는 거리, −0.005, −0.035..0.01) · `knee gate slack`(무릎 보정: 무릎 링 안쪽 변이 가랑이 너머로 들어와도 봐주는 거리, −0.005, −0.035..0.01) · `armpit gate slack`(겨드랑이 보정: arm 링 lo 변이 spine2 코너 아래로 내려와도 봐주는 거리, −0.01, −0.05..0.01; 음수 = 띠의 높이 요구) · `neck gate slack`(V 바닥 보정: `neck mid`가 `sternum` 아래로 내려와도 봐주는 거리, −0.02, −0.08..0.01; 음수 = V 높이 요구), 골반 `crotch drop`(0.08, 0..0.3) · `hip out`(비율 0.8, 0..2), knee 링 `out`(양 링의 바깥쪽 변 s 여유, 0.005, −0.1..0.1) · `back`(0.02, −0.05..0.2), ankle 링 `tilt`(45°, 0..80) · `front`(−0.08, −0.1..0.1) · `back`(0.005, −0.05..0.1; 발바닥 높이도 정한다), elbow 링 `hi`(양 링 윗변, 0.01, −0.05..0.1), 손 `wrist thumb`·`wrist pinky`(손목 링 폭 여유, −0.016 / −0.004, −0.05..0.05) · `thumb out`·`pinky out`(8각형 바깥 기둥 여유, −0.01 / −0.01, −0.05..0.05; 양손 공통, 음수 = 살 쪽으로), `finger out`(모든 손가락 링 양변의 perp 여유 일괄, 0.0005, −0.005..0.01; §4c) · `valley reach`(계곡 제어점을 손목 반대 방향으로 미는 거리, 0.011, 0..0.03; §2·§4b). 움직이면 재bake + 케이지 갱신만(와이어가 바로 따라옴); 메시는 이전 bind에 남고 `bind mesh` 경고가 뜬다 — 수 초 걸리는 bind는 그 버튼으로 따로 누른다(2026-09-14; 전에는 슬라이더를 놓는 순간 자동 bind였다). 값이 정해지면 diff를 `cage_tune.baked`에 더하고 씬의 diff를 0으로 돌린 뒤(`reset tune`) 이 표를 고친다. |
...
**케이스 검사** — `--case "<이름>"`(`results.csv`의 case 열 그대로, `--tiers`·`--skip`은 그 케이스가 든 층으로)은 한 케이스의 자기겹침 삼각형을 정점 번호·이름·위치(cm, spine 링 프레임의 side / up / depth)로 풀어 쓰고, 열린 지도 주변의 랜드마크(crown·head·L/R arm·정중선 기둥·왼발)를 함께 찍는다. 보고서는 그룹 이름까지만 말하므로 기전을 읽을 때 쓴다(2026-09-12, 지도 G·H). 무작위 케이스(`random#i`)는 이름이 없으므로 rest에서 ¼ 이상 벗어난 뼈를 함께 적는다. `--set "left clavicle=1.3, right clavicle=0.8, …"`은 이름한 뼈만 그 비로, 나머지는 rest로 둔 **임의 조합**을 같은 형식으로 검사한다 — 슬라이더로 옮기기 전에 재현 조합을 찾는 손잡이(2026-09-13, G′).
...
- **[N17] 정중선 기둥의 앞끝은 자기 띠에서 풀린다.** 정중선 기둥은 자기 띠(닫는 가로대의 링)의 깊이를 그대로 쓴다(§3b) — `neck mid`의 앞끝은 arm 링 hi 변의 `front` 여유가 정한 깊이에 놓이고, rest에서 두 값은 같다(둘 다 0.1186). 그런데 목·얼굴을 조이려면 arm 링의 hi `front`를 줄여야 하고, 그러면 V 바닥이 함께 뒤로 끌려 목 앞·쇄골 사이로 파고든다. 두 점이 요구하는 깊이가 다르다: arm·hi는 가슴 띠의 깊이(승모근 위), `neck mid`는 목 앞(울대) 깊이. 그래서 앞끝에만 자기 여유 `neck front`를 띠 위에 더한다 — 뒤끝은 목 뒤가 가슴 띠와 같은 깊이라 그대로 둔다. `sternum mid`도 같다: arm 링의 lo 변이 정하는 깊이는 겨드랑이의 것이고 흉골은 그보다 앞이므로 `sternum front`로 민다. 이 둘로 arm 링의 두 변을 살까지 당기고도 목 앞과 윗가슴이 덮인다 — rest 실측 0 정점 탈출. 대가는 가슴 띠의 두 가로대가 깊이로 13 cm 벌어진 것이고, chest 뼈를 줄이면 그 사각형이 안장이 되어 접힌다(§9 자기겹침 지도 B). 필요해지면 뒤끝도 같은 방식으로 푼다. **2026-09-09**: `sternum front`는 없어졌다 — sternum이 겨드랑이 선의 교점이 되어(`[N27]`) 흉골 앞은 arm 링 lo 변의 `front` 자체로 덮는다(0.055 → 0.08, 겨드랑이 앞 코너도 그만큼 나온다). `neck front`만 남는다. **2026-09-09 (2)**: 그것도 없어졌다 — `neck mid`가 몸통 공유 깊이를 받는다(`[N28]`). 이 노트가 풀던 문제(arm 링의 깊이를 조이면 정중선 기둥이 딸려 들어온다)는 arm 링에 자기 깊이가 없어지면서 뿌리째 사라졌다.
...
- **[N20] 맞추는 대신 지킨다 — 보정(gate).** 케이지의 모양은 **주어진 메시에 의존적**이다. 망치상어처럼 머리가 좌우로 긴 모델, 어깨에 돌출부가 있는 모델을 주면 "살에 꼭 맞춰 자기겹침을 없앤다"는 목표 자체가 도달 불가능해진다 — 어떤 메시는 언제나 여유를 빼앗아 간다. 그래서 목표를 바꾼다: **비상식적인 스켈레톤 변화에서 케이지를 지킨다.** 그런 변화가 실제로 요구되면 그때 케이지를 고도화해도 늦지 않다.
    ...
    - **여유가 없으면 목이 짧아지지 않는다.** 닿는 즉시 올리면 목뼈를 줄인 만큼 머리가 그대로 따라 올라와 편집이 상쇄된다(목뼈 셋 0.5에서 88.6 mm). 판이 실제로 접히기 시작하는 것은 링이 이음선 아래로 자기 높이의 5분의 2쯤 가라앉은 뒤다 — 측정: 여유 0·24·38·45 mm에서 chest+목 계열이 전부 0이고 **50 mm에서 4 삼각형**. 그래서 여유를 두고 경계 안쪽에서 튠한다(초기 0.038 = 목 3개 축소 시 들리는 높이 88.6 → 50.6 mm). **2026-09-12, 룰 8 뒤 3.0 → 1.0 cm.** 그 문턱은 rest 깊이에서 잰 것이었다 — 머리가 키를 따라 깊어지고(턱이 앞·아래로) arm 링이 쇄골을 따라 얕아지자, 좌우 쇄골이 다른 비례(4층 `torso 0.7 · arms 0.85 · legs 1.4` + 한쪽 0.9/1.1)에서 턱이 승모근 아래 1.3~1.5 cm에 든 것만으로 목 앞판이 접혀 머리 앞면을 뚫었다(§9 지도 G). 여유 0과 1 cm는 1·2·4층 모두 깨끗하고 탈출도 같았다; 목이 줄 여지를 1 cm 남기고 에디터에서 확인해 0.01로 정했다. driver가 들어온 뒤 gate 여유를 다시 본 첫 사례다.
    ...
    - **다섯째 보정: 겨드랑이는 절두체 위에 있다** (2026-09-12, §9 지도 H). 어깨 밑동과 upper thorax를 줄여 Arm 관절을 내리고 쇄골을 늘여 이음선을 키우면, 겨드랑이(arm 링 lo 변, 관절 아래 이음선 절반)가 Spine2 관절에 닿거나 넘는다. 절두체(`[N27]`)의 교점은 선분 밖이면 끝에 붙으므로 spine2 링의 네 코너가 겨드랑이 코너 그 자리가 되고, 그 사이 판·옆벽·가로대는 높이 0의 띠다 — 몸통 깊이를 공유하던 때는 같은 평면의 퇴화 사각형이라 겹침이 아니었지만, 복원(`[N29]`)이 spine2를 자기 폭 비로 arm 링을 쇄골 비로 따로 놓자 띠가 접힌 얇은 조각이 되어 몸통 반판과 상완 판을 뚫는다(3층 316건). 재는 곳은 겨드랑이 변, 세우는 것은 arm 링 전체(둘째 보정처럼 — 이음선이 남고 elbow는 제자리), 바닥은 spine2 링의 그쪽 코너. clamp된 상태에서 그 바닥은 겨드랑이의 옛 자리 그 자체라 `lift = |여유|` — 여유가 곧 벌어지는 띠의 높이이고, 그래서 **음수여야 한다**(초기 −0.01). 이 gate가 옮긴 어깨를 `head above arms`가 바닥으로 읽으므로 그보다 먼저 돈다. spine2의 코너는 옮기지 않는다 — 걸침이 이미 지난 뒤라 옛 겨드랑이 자리에 남고, 그것이 바로 원하는 띠의 아랫변이다. 남는 것: Spine1까지 겨드랑이를 넘는 조합이면 spine1 = spine2가 겹친 채 남는다 — 3층 20,000에 나오지 않았다.
    - **여섯째 보정: V는 높이를 갖는다** (2026-09-13, §9 지도 G′ V 바닥 14). chest(Spine3→Neck) 0.5는 Neck 관절과 `neck mid`를 6.5 cm 내리고, 어깨 밑동 1.3~1.5는 Arm 관절과 겨드랑이를 올리며, 짧은 쇄골은 이음선을 줄여 그쪽 겨드랑이를 더 올린다 — 겨드랑이 선의 교점인 `sternum`(`[N27]`)이 함께 올라 V 바닥을 넘는다(재현 `chest 0.5 · right clavicle 0.55 · left shoulder base 1.3 · right shoulder base 1.5`: neck mid 46.7 대 sternum 47.0). V의 높이가 0을 지나면 몸통 반판의 꼭대기 가로대 둘(arm hi–neck mid, arm lo–sternum)이 순서를 바꿔 목 판과 가슴 띠가 서로를 지난다. 재는 곳 = 올리는 것 = `neck mid` 두 끝, 바닥 = `sternum` 두 끝 — V넥 (3)은 바닥을 arm hi 변으로 예고했지만 접힘의 직접 조건은 sternum과의 순서라 그쪽을 잡았다. 이음선 변은 그대로고 기둥만 오르므로 V가 얕아질 뿐 모양은 남는다. 여유는 음수(V 높이 요구, 초기 −0.02); rest 차 12.6 cm라 no-op. sternum을 옮기는 gate가 없어 순서는 무관하다. 대가: chest를 아주 줄여도 V 바닥이 흉골 위 여유만큼에 멈춰 목 앞이 그만큼 남는다.
...
- **[N28] 몸통은 옆에서 보면 상자다 — `body front`/`body back`.** 사지와 머리를 뺀 몸통의 모든 링과 기둥이 Hips를 d 앵커로 하는 앞뒤 선 한 쌍을 공유한다: `L/R arm` 링 네 코너, `neck mid`(v72·v73), `spine` 링, `crotch`(v82·v83)·`L/R hip`. spine1·spine2와 그 mid, sternum은 절두체·교점(`[N27]`)이라 자동으로 같은 선 위다. 값은 몸통 살(Hips 서브트리 − 사지·머리 서브트리) 전체의 `depth` 구간 inflate + 튠 둘. **왜.** 깊이 복원(`girth_d`)이 몸통에 들어오기 전에 곱할 대상을 하나로 만든다 — 링마다 재고 튠한 깊이 아홉(arm hi/lo front/back, neck front, spine front/back, pelvis front/back)이 있으면 곱한 뒤에도 서로 어긋나고, 두 정중선 기둥의 앞끝을 띠에서 풀던 `[N17]` 같은 보정이 자리마다 생긴다. 하나면 driver도 하나다. **대가.** (1) arm 링이 몸통만큼 깊어진다 — 겨드랑이·승모근 코너가 가슴·엉덩이 선까지 나가고 상완 판은 그 상자에서 elbow 링으로 좁아지는 쐐기가 된다. (2) 허리·골반의 깊이가 가슴·엉덩이 최대치라 배 앞·허리 뒤가 넉넉히 뜬다 — 옆구리가 `[N21]`에서 4~6 cm 뜨는 것과 같은 종류이고, 잘록함은 케이지가 아니라 MVC의 몫이다. (3) 목 앞(`neck mid`)이 가슴 선까지 나간다 — V 바닥이 목이 아니라 가슴 상자의 윗면 정중선이 된다. 코드로는 `recipe.body` 플래그 하나(measure가 d 앵커와 네 코너 여유를 몸통 값으로 덮는다)와 `neck mid`·`pelvis_post`의 d 인자. 튠 아홉이 둘로 줄었다.
- **[N29] 깊이 복원 — 구간마다, 자기 폭에서.** 룰 8. 두께 driver의 나머지 반이다: 룰 2~7이 실루엣을 뼈에 물렸고(§6 `girth`), 깊이는 몸통 상자(`[N28]`)에 남아 있었다 — 어깨가 1.5배 넓어져도 가슴은 rest 두께다. **무엇을 복원하는가.** 레시피가 변형한 변(실루엣 = 폭)을 기준으로 변형하지 않은 변(깊이)을 고쳐 rest의 깊이/폭 비를 되찾는다: `깊이 := rest 깊이 · 폭 / rest 폭`, 여유마다 자기 앵커에서. 폭은 링 자신이 놓인 뒤 읽는다 — 그래서 span을 선언하는 `girth_d`가 아니라 **패스**다. spine 링의 폭은 걸친 변이, spine1·spine2의 폭은 절두체가, arm 링의 폭은 쇄골이, 고관절 링의 폭은 고관절 뼈와 `hip out`이 정하는데 그중 span 하나로 적을 수 있는 것은 arm뿐이다. **왜 링마다 독립인가.** 몸통 공유 깊이와 절두체는 rest를 하나의 상자로 만드는 **놓는 규칙**이고, 복원은 그 뒤에 온다. 어깨만 넓어지면 arm 링만 깊어져야 하고, 그 아래 spine2가 절두체라는 이유로 함께 깊어지면 가슴이 어깨를 따라가는 것이 아니라 상자가 통째로 커지는 것이다. 구간마다 자기 폭을 보면 상자는 편집이 있는 자리에서만 갈라진다. **자리는 Hips.** 링 식은 `앵커 + 여유·g_d`이고(§6) 복원은 그 `g_d`를 자기 폭의 비로 두는 것과 같다 — 같은 모델이라 설명이 하나이고, spine1·spine2도 rest에서 몸통 선 위에 있으므로 Hips 기준 여유가 `body front`/`back` 그대로다. 링 중심을 자리로 삼으면 앞뒤가 같은 양으로 벌어지는데, 배 앞(`body front`)과 등 뒤(`body back`)의 비를 유지하는 쪽이 "두꺼운 몸"에 가깝다. **기둥은 평균.** 링이 깊어지면 그 위의 기둥이 따라와야 한다(§9 "driver의 단위는 구간"). 두 구간 사이의 기둥(`neck`·`sternum`·`crotch`)은 어느 한쪽에 속하지 않으므로 양쪽 코너의 평균으로 두고 — 대칭 편집에서는 교점(`[N27]`)과 같은 값 — 한 링 위의 기둥(`spine`·`spine1`·`spine2` mid)도 같은 형식으로 자기 링의 변에 둔다. 교점(§6 3)은 복원 전 코너를 읽었으므로 깊이만 다시 정한다. **gate와 직교.** 복원은 `depth`만, 보정 넷은 `up`·`side`만 옮기므로 순서를 바꿔도 결과가 같고, driver → gate의 순서(§9)는 그래서 형식상의 것이다. **rest 항등.** `rest_width`를 그 rest 케이지에서 굽고 rest에서 `by = 1`, 기둥 평균도 rest에서는 놓인 자리 그대로다 — bind와 지금까지의 측정이 전부 유효하다. **대가.** (1) 몸통 상자가 편집 뒤 링마다 다른 깊이가 되어 옆에서 본 실루엣에 단이 생긴다 — 그것이 의도다. (2) spine 셋의 폭은 겨드랑이·고관절이 정하므로 chest·pelvis 뼈를 늘여도 깊이는 안 바뀐다 — 척추 길이는 두께의 근거가 아니라고 놓은 것. (3) `--probe`가 복원을 몰라 복원 대상 링의 깊이 행이 두 번 곱해져 ~2.1로 읽힌다(§7b). **사지도 같은 규칙이다 (같은 날).** elbow·knee·ankle·toe 링과 발끝 기둥 쌍이 구간이 되고, 손목 링과 손은 빠진다 — 손목 이하는 세 축이 이미 선언되어(`[N25]`) 되찾을 것이 없다. 사지 링의 폭은 `girth`(뿌리 뼈)가 정하므로 `girth_d = girth`와 결과가 같지만, 선언은 구간 하나로 둔다. **`by`는 폭 비이고 여유는 각자의 앵커에서 곱한다.** 처음에는 `by = rest 비 · 폭 / 깊이`로 놓인 깊이를 자리 하나 기준으로 곱했다. 그것은 놓인 깊이 여유가 rest 그대로일 때만 같은 식이고, toe 링·tip은 위가 ToeBase에 아래가 Foot에 걸려 있어(당시 정의) 발 뼈가 길어지면 ToeBase가 내려가 놓인 깊이가 먼저 줄었다(legs ×1.4: 5.96 → 4.62). `by`가 1.4가 아니라 1.81로 부풀어 발바닥을 ToeBase 아래 5 cm까지 밀고, ankle은 1.4라 뒤꿈치가 발가락보다 2.2 cm 높이 떠 있었다(에디터에서 확인, 2026-09-12). 그래서 `by = 폭 / rest 폭`, 앞 정점은 front 앵커 관절에서, 뒤 정점은 back 앵커 관절에서 곱한다 — `g_d`의 정의 그대로. 발은 그 뒤 정의를 바꿔 높이를 전부 Foot에서 재므로(`[N30]`) 두 자리가 다시 같은 관절이 되었지만, 선언은 둘로 남긴다. **발의 자리가 Foot인 결과**: 뒤꿈치 코너는 기울어진 d를 따라 `back·(by−1)·sin 45°` = `(by−1)·floor` 내려가고 toe·tip의 바닥은 up을 따라 정확히 같은 `(by−1)·floor` 내려간다 — 발바닥이 수평인 채로 통째로 내려가고, 발바닥 두께가 스케일을 따라 커진다. 세 구간의 `by`가 같은 것은 셋의 폭이 모두 고관절 뼈의 비이기 때문이다. **버린 둘.** ToeBase를 자리로 — 발볼 아래와 뒤꿈치 아래가 다른 길이라 legs ×1.4에서 뒤꿈치가 발볼보다 1.34 cm 낮아진다(`(by−1) × 3.36`, Foot→ToeBase 낙차). 바닥(`back` 정점)을 자리로 — 발바닥은 고정되고 발등만 오르는데, 그러면 발바닥 두께가 스케일을 따르지 않는다. 대가는 발등이 거의 오르지 않는 것(toe 윗변이 rest에서 Foot +0.5 cm라 ×1.4에서 +0.2 cm) — 두꺼운 발은 아래로 두꺼워진다. **머리 링 둘도 같은 날 들어왔다** (자리 Head, mid 둘은 평균): 폭이 키(`[N23]`)이므로 깊이도 키의 비다 — 키의 61%가 다리라(`[N23]` 측정) 다리를 늘이면 머리가 깊어지고, 쇄골은 머리에 아무 영향이 없다. 그것이 룰 4의 뜻 그대로인지는 폭의 근거를 다시 볼 때 함께 본다.
- **[N30] 발의 높이는 전부 Foot에서 잰다.** toe 링의 윗변과 tip의 위끝은 잰 자리가 ToeBase 주변의 살이지만, 걸리는 관절은 아래(`floor`, `[N14]`)와 같은 **Foot**이다 — 잰 높이에서 `Foot − ToeBase`의 `up` 낙차(3.36 cm)를 빼서 Foot 기준 여유로 굽는다. **왜.** 이 rig의 발 뼈(Foot→ToeBase)는 앞으로만이 아니라 아래로도 14° 기울어 있어, 길어지면 ToeBase가 길이의 0.247배 내려간다 — legs ×1.4에서 1.34 cm, ×1.62면 발바닥 평면에 닿는다. 위가 ToeBase를 따르면 그 단면이 수평인 발바닥에 눌려 발 뼈 길이에 따라 두께가 변하고(복원 전 5.96 → 4.62), 복원은 그 찌그러진 깊이를 기준으로 삼아 발바닥을 밀어냈다(`[N29]`). 발 전체를 발목에 매달린 한 판으로 두면 발 뼈는 발을 **앞으로만** 길게 하고, 두께는 고관절 비(복원)만 따른다. **버린 것.** 발볼 전체를 ToeBase에서 재기(아래도 ToeBase − 2.10) — rig의 관절 위치에는 충실하지만 발 뼈를 늘이면 발볼이 뒤꿈치보다 내려가 지면과 어긋난 발이 된다. 위만 ToeBase, 아래는 Foot(전 정의) + 앵커별 복원 — 결과는 수평이지만 "왜 위만 ToeBase인가"에 답이 없다. **대가.** 발 뼈만 아주 길게 하면 스켈레톤의 ToeBase 관절이 케이지 발바닥 가까이(×1.5에서 0.4 cm 위) 내려가지만 관절은 그려지지 않고 스키닝도 없어 살에는 보이지 않는다 — 변형된 스켈레톤을 다른 데서 쓸 때 볼 것.
- **[N31] 발 안쪽 옆판의 대각선은 바깥쪽을 따른다.** `strip`은 옆판 쿼드의 대각선을 사슬 진행 방향에 매어 긋는다 — 도착 기둥의 앞 정점과 출발 기둥의 뒤 정점(§5d). 사슬 2는 다리 바깥쪽을 무릎→발볼로 내려가고 안쪽을 발끝→무릎으로 되올라오므로 같은 모양의 쿼드 둘이 다른 대각선을 갖는다: 바깥은 발볼 앞(v48)–뒤꿈치(v45), 안쪽은 발등(v47)–발바닥(v50). rest에서는 쿼드가 거의 평면이라 차이가 없다. 고관절 1.5 + 발 뼈 0.5(지도 F)에서 룰 8이 발등 코너를 Foot 앞 7.84 cm로 밀고 toe 평면은 6.58 cm에 서서 발등이 toe 평면을 1.3 cm 넘는다 — 발등–발바닥 대각선의 삼각형(발등·발바닥·발볼 앞)은 toe 평면 앞쪽에서 발바닥 높이까지 내려와 발가락 옆벽과 겹치고, 발볼 앞–뒤꿈치 대각선의 삼각형(발등·뒤꿈치·발볼 앞)은 toe 평면 앞쪽에서 발볼 앞 위로만 남아 비켜 간다. 바깥쪽이 안 깨지는 이유이고, 그래서 안쪽 두 쿼드만 대각선을 돌린다(§5b `turned`). 거울 규칙(`[N3]`)은 오른쪽을 왼쪽의 거울로 만들 뿐이라 잘못도 거울로 옮기므로 오른발도 함께 적는다. gate나 자리 변경 없이 토폴로지만으로 닫힌다 — 발등이 toe 평면을 넘는 것 자체는 남지만 껍질은 닫힌 채다. **일반화하지 않은 이유**: 대각선을 사슬 방향과 무관하게 정하는 규칙(정점 번호 순 같은 것)은 몸 전체의 쿼드를 바꿔 지금까지의 포함·자기겹침 측정을 무효로 한다. 대각선을 이름한 첫 행이므로 §5d의 "표는 고리와 사슬만 선언한다"에 예외로 적었다.
...
- **자기겹침 지도 — 전부 닫혔다, 3층까지** (2026-09-13, 여섯 보정). 2026-09-07, 네 보정이 모두 든 케이지로 잰 스윕(§7b, 손을 rest에 묶고 seed 1):
    ...
    | **룰 8 깊이 복원 (09-12)** | **0 밖 · 겹침 0** | **0** | **2** (F) | 3층 안 돌림 · 4층 1,125 **0** — 새는 케이스 274 → **8**, 최악 22 → **2** |
    | **룰 8 + 머리 링 (09-12 뒤)** | **0 밖 · 겹침 0** | **0** | **2** (F) | 3층 안 돌림 · 4층 1,125 **4** (G) — 새는 케이스 10, 최악 2 |
    | **+ 발 대각선 `[N31]` + head gate 1.0 (09-12 끝)** | **0 밖 · 겹침 0** | **0** | **0** | 3층 20,000 **414** (H 316 · G′ 84 · 둘 14; 09-07 이후 첫 3층) · 4층 1,125 **0** — 새는 케이스 9, 최악 1 |
    | **+ gate `arm above spine2` (09-13, export 사본에 심어 잼)** | **0 밖 · 겹침 0** | **0** | **0** | 3층 20,000 **98** — H **0**, 남은 98은 전부 G′ · 4층 1,125 **0**; 탈출 변동 없음 |
    | **+ `arm gate slack` −0.03 (09-13, 씬 재bake·export `out_gates5`)** | **0 밖 · 겹침 0** | **0** | **0** | 3층 20,000 **14** — 전부 V 바닥 · 4층 1,125 **0**; 새는 케이스 1·2·4층 270 → **206**, 3층 12,719 → **11,679** |
    | **+ gate `neck mid above sternum` (09-13, export 사본에 심어 잼)** | **0 밖 · 겹침 0** | **0** | **0** | 3층 20,000 **0** · 4층 1,125 **0**; 탈출 변동 없음 — **네 층 모두 0** |
    | **씬 재bake·export로 여섯 gate 확정 (09-14, `out_scene6`)** | **0 밖 · 겹침 0** | **0** | **0** | 3층 20,000 **0** · 4층 1,125 **0**; 탈출 11,885 케이스, 중앙값 21 · 90번째 88 · 최악 271 |

    판정 둘(rest 포함, 자기겹침)이 모두 0이다. 09-12 룰 8 뒤 쌍 2건(F, 고관절 1.5 + 발 뼈 0.5)이, 머리 링까지 들어간 뒤 **비례 4층 4건**(G, 짧은 몸통 + 긴 다리)이 열렸고, 같은 날 F는 토폴로지(`[N31]`)로, G는 `head gate slack` 1.0 cm로 닫혀 마지막 스윕(`out_slack1`)은 1·2·4층에서 다시 **0 / 0 / 0**이다. 그러나 09-07 이후 처음 돌린 **3층(무작위 20,000)은 414건**이 깨졌다 — 새 기전 H(316)와 G′(84+14). 룰 8 전의 3층은 0이었으므로 이것이 룰 8이 3층에서 요구하는 값이다. H는 09-13 gate `arm above spine2`로 닫혔고(3층 98), G′는 같은 날 `arm gate slack` −0.03과 gate `neck mid above sternum`으로 닫혔다 — 09-14 재bake한 씬의 export로 돌린 스윕(`out_scene6`)이 **네 층 모두 0**을 확정했다. 아래 표는 무엇이 있었고 무엇이 닫았는지의 이력이다 — 새 기전이 나오면 여기에 이어 적는다. 09-09 스윕(`out_mid_body`)의 탈출은 새는 케이스 466 → 266으로 줄고 최악은 27 → **57**로 올랐다 — 한쪽 고관절·쇄골 0.5 같은 극단 조합에서 사지 링이 절반으로 죄어진 자리(룰 3)이며, 비례 4층에서는 실패 0. 손 뼈까지 훑은 7,061 케이스(`out_mid`)에서는 중수골 0.5에서 손가락 링끼리 겹치는 실패 1,771이 나왔다 — 손은 이전에 훑은 적이 없어 비교 대상이 없고, 두께 driver의 자기겹침 상한이 손에서는 아직 안 잡힌 것이다.
    ...
    | **F 발등이 발볼을 지난다** (2026-09-12; 토폴로지로 닫음 `[N31]` — 에디터 확인, 스윕 `out_slack1` **0**) | `L/R ankle+L/R toe` 1·1 · `L/R tip+L/R toe` 1·1 (3 삼각형씩) | 고관절 1.5 **+** 발 뼈 0.5 (각각 단독은 무사) | 룰 8이 ankle 링의 앞 여유를 45° 기울어진 d를 따라 ×1.5 하면 발등 코너가 Foot 앞 7.84 cm에 서는데, 발 뼈 0.5의 toe 링 평면은 Foot 앞 6.58 cm다 — 정강이–발등 판이 toe 링을, 발등–발끝 뚜껑이 tip을 뚫는다. ankle의 "깊이"가 앞뒤와 위아래를 함께 담은 축이라 복원이 발등을 앞으로도 민다. **닫음**: 발 안쪽 ankle–toe 옆판 쿼드의 대각선을 바깥쪽처럼 발볼 앞–뒤꿈치로 돌린다(§5b `turned`) — 발등이 toe 평면을 넘는 것은 남지만 그 삼각형이 발가락 옆벽 위로 비켜 간다 `[N31]`. gate·자리·범위 후보는 필요 없어졌다 `[N29]` |
    | **G V 삼각형이 머리 판을 뚫는다** (2026-09-12, **4층** 닫음 — `head gate slack` 1.0 cm; **3층에는 G′가 남는다**) | 몸통 반판 꼭대기 `L/R arm+neck mid+sternum mid` 2·2 · 머리 판 `crown+head`·`crown(mid)+head(mid)` 4·2·2 (2~4 삼각형) | `torso 0.7 · arms 0.85 · legs 1.4` + 좌우 한쪽 0.9/1.1 (좌우 1·1은 통과) | 머리 깊이 복원이 열었다(직전 스윕은 통과). 키 비 1.124로 머리가 세 축 모두 커지는데 목·척추는 30% 짧다. head 링의 턱 쪽 여유는 25° 기울어진 d를 따라 앞·아래로 커지고, `head above arms`는 head 링 최하단을 arm hi 변 −3.0 cm(씬 튠)까지만 지키므로 커진 머리 판이 V 삼각형과 만난다. `--case`로 읽은 기전(§7b): 턱 코너가 승모근 점 아래 1.5 cm(여유 3.0 안)에 있고, 한쪽 쇄골만 달라 두 arm 링의 앞 깊이가 13.1 / 14.4로 갈리자 그 평균을 받는 `neck mid`(13.8)가 몸통 꼭대기 삼각형을 머리 앞면(13.2)을 가로지르게 기울인다 — 대칭이면 셋이 다 13.1이라 스치지 않는다. **닫음**: `head gate slack` 3.0 → **1.0 cm** — 턱이 승모근 밑으로 1 cm 넘게 못 들어오면 목 앞판이 접히지 않는다. 여유 0·1 cm 둘 다 1·2·4층 깨끗, 탈출 동일; 에디터에서 1 cm로 확정 `[N20]` `[N29]`. **G′ — 3층(같은 날 밤)에서 84+14건이 남는다**: 쇄골 비대칭이 더 크면(`random#1324`: L 쇄골 1.28 / R ~1, 목 0.71 → arm 앞 19.8 / 13.3, `neck mid` 16.5, 머리 앞면 13.7) 턱이 여유 1 cm 안에 있어도 V 삼각형이 6 cm의 깊이 차를 가로지른다. 여유로는 안 닫힌다 — `neck mid`의 깊이 규칙(두 arm 링의 평균)이나 머리 앞면과 V의 관계를 봐야 한다. **그중 14건은 다른 접힘이다**(`random#1035` 13 삼각형: chest 0.52 · 쇄골 R 0.55): Neck 관절이 겨드랑이 선까지 내려와 **V 바닥(`neck mid` 47.9)이 `sternum`(48.0)과 같은 높이**가 되고, 높이 0의 V에서 목 판(`L/R arm+head+neck mid`)과 몸통 꼭대기 가로대(`arm+spine2 mid+sternum`)가 접힌다 — V넥 (3)이 예고한 `neck mid` gate(올리는 것 `neck mid`, 바닥 L·R arm hi 변, 여유 = rest 차)의 자리. **깊이 비대칭 84는 `arm gate slack` −1 → −3 cm로 닫혔다**(2026-09-13, `[N20]` 둘째 보정): 재현 조합 `left clavicle 1.3 · right clavicle 0.8 · right shoulder base 1.5 · lower neck 0.7 · legs 1.4` — 머리가 얕은 쪽 어깨에 얹히고 그 팔 링이 머리 옆에 붙어야 하며, 쇄골만 0.5 / 1.5로는 깊은 쪽이 곧 높은 쪽이라 나지 않는다. **V 바닥 14는 gate `neck mid above sternum`(§6b 여섯째 보정)으로 닫혔다** — 재현 `chest 0.5 · right clavicle 0.55 · left shoulder base 1.3 · right shoulder base 1.5`(neck mid 46.7 대 sternum 47.0) → 0, 3층 20,000 **0**. G 계열 전부 닫힘 |
    | **H 절두체가 겨드랑이에 닿는다** (2026-09-12, **3층**; gate `L/R arm above spine2`로 **닫음** — 3층 316 → **0**, 1·2·4층 변동 없음; 씬 재bake 전) | `spine1 mid+spine2+spine2 mid` 297 · `spine1+spine2` 213 · `L/R arm+spine2` 165·155 · `L/R arm+L/R elbow` 167·149 (최악 `random#2459` 20 삼각형) | 척추 위쪽이 길고(lower thorax 1.35) 그 위가 짧아(upper thorax 0.54 · chest 0.71 · shoulder base 0.58) Spine2 관절이 겨드랑이 선(arm 링 lo 변, 높이 26.0)에 닿거나 넘을 때 | 절두체의 교점(`crossing`)은 선분 밖이면 끝에 붙는다(clamp01) — spine2 링 네 코너가 겨드랑이 코너 그 자체가 되어(26.0 = 26.0) 그 사이 몸통 판·옆벽·`spine2 mid`–`sternum` 가로대가 높이 0인 띠가 된다. 룰 8 전에는 그 띠가 같은 평면의 퇴화 사각형이라 겹침으로 잡히지 않았을 것이나(09-09 이후 3층을 돌린 적이 없어 확인은 못 했다), 복원이 spine2의 깊이를 자기 폭 비로(20.9) arm 링을 쇄골 비로(20.4) 따로 놓자 띠가 접힌 얇은 조각이 되어 몸통 반판·상완 판을 뚫는다. spine1이 그 높이에 오면 같다. 후보: gate `spine2 below armpits`(spine2 링 + mid를 arm lo 변 아래로, `spine above hips`처럼 링째, 음수 여유; spine1도) / 절두체 평면을 겨드랑이 아래로 clamp(놓을 때 — 그러면 걸침이 판단을 하게 된다, `[N20]`) / 그 조합을 길이 범위 밖으로 `[N29]` |
...
- **두께 driver — "키가 크면 두꺼워진다". 룰을 하나씩 세우는 중.**
    ...
    - **깊이 복원 전 정리** (2026-09-09, `[N27]`): 독립 깊이를 갖던 정중선 기둥 셋을 걷어냈다 — `sternum mid`는 겨드랑이 선의 교점, spine1·spine2와 그 mid는 겨드랑이 → spine 링 절두체의 중간 링. 남은 독립 정의는 `neck mid`의 높이(Neck 관절)와 구운 오프셋 방식의 `spine`·`crown`·`head` mid. **같은 날 (2)**: 몸통의 깊이를 `body front`/`body back` 하나로 묶었다(§3f, `[N28]`) — arm 링·neck mid·spine 링·골반 기둥이 옆에서 보면 한 선이고, 깊이 튠 아홉이 둘로 줄었다. 깊이 복원의 몸통 쪽 대상은 이제 이 값 하나다 — 룰 8이 그것을 링마다 자기 폭 비로 되찾는다.
    - **룰 7 — 가랑이는 고관절 너비만큼 내려온다** (2026-09-09, `[N26]`). 골반 기둥 셋의 `girth` = LeftUpLeg → RightUpLeg(`side`): crotch의 drop과 고관절 기둥의 `f·drop`이 같은 비를 받아 기울어진 고관절 링의 정의가 유지된다. 두 고관절 뼈의 합이라 비는 둘의 평균 — 비대칭 편집에서 절반씩. 깊이(앞뒤)는 룰 8이 되찾는다. 씬 재bake·스윕은 아직이다.
    - **룰 8 — 깊이는 자기 폭에서 되찾는다** (2026-09-12, `[N29]`). 복원 패스(§6): 걸침 뒤, 보정 앞. 몸통 구간 일곱 — `spine`·`spine1`·`spine2`, `L/R arm`, 기울어진 고관절 링 둘(`L/R hip` ↔ `crotch`) — 이 각자 rest의 깊이/폭 비로 깊이를 다시 놓고(Hips 기준 여유 × 자기 폭 비), 그 위의 기둥 여섯(`neck`·`sternum`·`spine`·`spine1`·`spine2` mid, `crotch`)은 닫는 코너들의 깊이 평균을 받는다. 몸통 공유 깊이(§3f)는 rest의 것이 되고, 편집이 있는 자리에서만 상자가 갈라진다. **사지도 같은 날**: elbow·knee 링(자리 = 자기 관절), ankle·toe 링과 발끝 기둥 쌍(자리 = 셋 다 **Foot** — 발바닥이 수평으로 내려가고 발바닥 두께가 스케일을 따른다; 그러기 위해 toe 윗변·tip 위끝의 d 앵커를 ToeBase에서 Foot으로 옮겼다 `[N30]`). 손목 이하는 빠진다(`[N25]`). `by`는 폭 비, 여유는 각자의 앵커에서 곱한다 — 첫 구현(`rest 비·폭/깊이`, 자리 하나)은 legs ×1.4에서 뒤꿈치가 뜨고 발가락이 내려가는 것으로 틀렸음이 드러났다. 머리 링 둘(자리 Head, mid 둘은 평균)은 같은 날 뒤에 들어왔다 — 구간 19, 기둥 평균 8. 그 스윕(`out_restore_head`)은 rest 0 / 0, F 2건 그대로, 탈출 같은 자리 — 그리고 **4층에서 4건**(지도 G: 짧은 몸통·긴 다리에서 커진 머리 판이 V 삼각형과 만난다)이 깨졌다. 룰 8 뒤 첫 비례 실패이자 상한을 넘은 것. **같은 날 둘 다 닫았다**: F는 발 안쪽 옆판의 대각선(`[N31]`), G는 `head gate slack` 3.0 → 1.0 cm(`[N20]`) — driver 뒤 gate 여유를 다시 본 첫 사례. 마지막 스윕(`out_slack1`, 2,321 케이스): rest 0 / 0, **실패 0**, 4층 새는 케이스 9 · 최악 1; 극단 쌍의 탈출은 그대로(최악 158, 고관절 0.5 + 발 뼈 1.5). **3층(20,000, 09-07 이후 첫 재실행)은 414건 실패** — H(절두체가 겨드랑이에 닿아 접힘, 316)와 G′(큰 쇄골 비대칭에서의 G, 84+14). 룰 8 전의 3층은 0이었으므로 이것이 룰 8의 3층 값이다 — 새는 케이스 12,718, 중앙값 21, 최악 270. **H는 다음 날 gate `L/R arm above spine2`(§6b 다섯째 보정)로 닫혔다**: 3층 414 → 98, 남은 98은 전부 G′, 1·2·4층과 탈출은 변동 없음. **G′의 깊이 비대칭 84는 `arm gate slack` −3 cm로 닫혔다**(3층 98 → 14, 탈출도 줌). **V 바닥 14는 gate `neck mid above sternum`으로**(3층 **0**). 룰 8이 연 것은 전부 닫혔다 — gate 둘(겨드랑이·V)과 여유 둘(머리·어깨)로. **스윕(09-12, `out_restore`)**: rest 0 / 0. **비례 4층은 사실상 닫혔다** — 새는 케이스 274 → 8, 최악 22 → 2, 룰이 겨냥한 자리다. 극단 쌍은 나빠졌다 — 한쪽 고관절 0.5가 그 다리의 링을 폭에 이어 깊이까지 절반으로 죄어 종아리가 148까지 샌다(전 57; 판정은 아니다). 자기겹침 **2건**이 새로 열렸다 — 지도 F(고관절 1.5 + 발 뼈 0.5, 발등이 toe 평면을 지난다), 상한에 걸린 것이므로 다음에 닫는다. probe는 복원을 몰라 복원 대상 링의 깊이 행이 ~2.1로 읽힌다(§7b).
    ...
    - **룰 하나씩 세운다 — 무엇이 판정하는가.** driver는 gate와 달리 "고칠 실패"가 없는 자리에서 출발한다(판정 둘이 이미 0). 그래서 룰을 넣기 전에 기준을 먼저 둔다.
        ...
        - **지금 gate 여유 넷은 driver가 없는 폭에 대해 튠된 값**이므로, driver가 들어오면 함께 다시 본다. 첫 사례: `head gate slack` 3.0 → 1.0 cm(2026-09-12, 지도 G, `[N20]`).
...
- **V넥 다듬기.** (1) arm·hi의 높이는 아직 어깨 관절 평면에서 잰 삼각근 정점 높이 — 승모근 능선에 딱 맞추려면 변별 측정 창이 필요. (2) 가슴 띠와 V 바닥의 깊이는 이제 몸통 공유 깊이 하나다(§3f, `[N28]`) — 가슴이 새면 `body front`, 등이 새면 `body back`. arm 링·neck mid에 자기 깊이 튠이 없다. (3) `neck mid`의 높이는 **Neck 관절 그 자리로 확정**(2026-09-09) — V 라인이 목 판과 몸통 판을 가르는 자리이므로 목뼈를 따라야 하고, 승모근 선의 교점으로 내리지 않는다. 깊이는 몸통 공유(§3f). chest를 줄여 V 바닥이 내려가는 자기겹침은 **gate `neck mid above sternum`이 막는다**(§6b, 2026-09-13 — 바닥은 arm hi 변이 아니라 `sternum`이다: 접힘의 직접 조건이 V의 높이 ≥ 0이기 때문). (4) head 링의 tilt·offset(25°, 0.023)은 head splitter에서 읽은 값 그대로 확정(2026-09-14, §7) — 씬의 splitter 오브젝트는 지워도 된다. (5) 목을 줄일 때 이음선이 턱선을 넘는 것은 아직 열려 있다(§9 자기겹침 지도 A).
...
- **골반 다듬기.** (0) spine 링이 고관절을 넘던 자기겹침은 §6b `spine above hips`가 닫는다 `[N16]`; 그 바닥이 `hip out`·`crotch drop`에 딸리므로 그 둘을 튠하면 §7b 스윕을 다시 돌린다. (1) `crotch drop` 0.08 · `hip out` 0.8은 에디터에서 튠해 확정(2026-09-14, §7); 골반·spine의 깊이는 몸통 공유 `body front`/`back`(§3f)이라 따로 튠하지 않는다. (2) 골반 판 깊이가 Hips·UpLeg 살 전체의 구간이라 crotch 정점도 엉덩이 깊이를 갖는다; 안쪽 허벅지 벽이 헐거우면 crotch만의 깊이(가랑이 근처 살)로 좁힌다. (3) 골반 판이 허리 링 깊이 ↔ 골반 깊이 보간이라 엉덩이 최대 돌출이 새면 `pelvis back`으로 받는다. (4) `L/R hip` 높이는 `hip out` 하나로 옆·위가 함께 정해진다 — 따로 필요하면 up 오프셋 열 추가.
...
- **발 다듬기.** (1) `ankle tilt` 45° · `front` −0.08 · `back` 0.005는 에디터에서 튠해 확정(2026-09-14, §7). (2) ankle 링의 joint slab이 넓어 발볼·정강이 살까지 단면에 들어올 수 있다 — 단면이 헐거우면 ankle 전용 측정 창(예: 평면 ± 발 두께)으로. (3) toe·tip의 위쪽·폭에는 아직 여유 열이 없다(잰 값 + margin) — 발등이나 발가락 끝이 새면 `front`/`out` 튠 추가. (4) tip의 위끝이 발가락 살 전체의 `up` 최대라 뚜껑이 발볼 높이다; 끝만큼 낮추려면 tip 근처 살로 잰다.
- **길이 범위 확정.** 스윕(§7b)은 슬라이더 범위인 rest × [0.5, 1.5] 전체를 훑는다. 실제로 지원할 범위가 정해지면 그에 맞춰 좁히고, 범위 밖으로 밀려난 실패는 기록만 남긴다.
- **bind 비용.** 제어점 40 → 232로 늘면서 MVC bind가 그만큼 무거워졌다(rest 메시당 1회, 수 초). 에디터에서는 `bind mesh` 버튼으로 따로 누르고(§7), 데모는 구운 bind를 싣는다. 좌표 방식은 MVC로 확정 — 두께는 케이지가 선언하고 좌표계에 맡기지 않는다([cage-deformation-plan.md](cage-deformation-plan.md)); 오목부의 음수 가중치가 문제로 드러나면 PMVC/QMVC가 후보다.
