...
알려진 한계도 확인 — 이 plus 자형 케이지는 오목해서 표본 대부분에 **음수 가중치**가 나온다(겨드랑이·가랑이). 당시에는 두께 붕괴와 함께 Green으로 넘어갈 이유로 적었으나, 두께는 케이지의 일로 정리되어(위 "채택 방향") 남는 것은 음수 가중치 하나다 — 드러나면 PMVC/QMVC.
...
2. **오목부**(겨드랑이·가랑이·손가락 사이)는 인체 케이지에 필연. MVC는 여기서 음수 가중치와 비국소성을 갖는다(cage.md `[N18]`). 지금까지의 스윕에서 판정을 깨지는 않았고, 드러나면 PMVC/QMVC로 음수 가중치만 없앤다.
...
4. **두께는 좌표계가 주지 않는다 — 그것이 설계다.** 뼈를 늘여도 단면은 케이지가 선언한 만큼만 두꺼워진다. 그 선언이 두께 driver이고, 룰 여덟(실루엣은 뿌리 뼈·키, 깊이는 자기 폭에서 복원)이 들어가 있다(cage.md §9). 두께 복원을 좌표 알고리즘에 맡기지 않는다는 결정은 확정이다(2026-09-14).

---

## 검증 (end-to-end)

원래 Green 채택을 전제로 세운 단계들이다. MVC 유지 뒤에는 3이 뒤집힌다 — 두께를 좌표계가 보존하지 않는 것이 요구이고, 실제 검증은 `cage_sweep --probe`의 전달비(0.92~1.03)와 네 층 스윕이 맡았다(cage.md §7b·§9).
1. **Identity**: 변형 케이지 == rest → 메시 불변. MVC 1e-15로 확인(위).
2. **Similarity 재현**: 케이지에 균등 스케일/회전/이동 → 메시가 동일 변환. linear precision으로 성립.
3. **Stretch(핵심)**: 한 축으로 케이지 늘이기(뼈 길이 증가 모사) → 단면은 케이지가 선언한 만큼만 따라온다. 선언의 도착률이 전달비다.
4. **Bend/Concavity**: 사지 굽힘 → 심한 self-intersection·음수가중치 아티팩트 없는지. 케이지가 rest pose에 서고 애니메이션은 재바인딩 위에 얹히므로 지금 파이프라인에는 해당 없음.
