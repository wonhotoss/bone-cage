...
격자 1,125. 현재(2026-09-14, 보정 여섯): rest 0 밖 · 0 겹침, 네 층 모두 실패 0. 변형 후 탈출은 판정이
아니라 통계로 남긴다([docs/cage.md §9](docs/cage.md)).
...
- **전역 케이지 하나.** 케이지는 rest pose에서 세워지고 현재 포즈를 모른다. 사상이 포즈에 무관하고
  ...
  골반을 따라간다.
- **두께 driver.** 룰 여덟이 들어갔다. 남은 미결 — 지원할 길이 범위, 손, 룰 8이 좁은 어깨에 하는 일의
  눈 확인 — 은 [docs/cage.md §9](docs/cage.md)에 있다.
